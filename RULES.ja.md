...
448ルール、36グループ。各ルールは一意のID、重要度、クラスで定義されます。
...
| ARC_002 | ファイルをUTF-8で読み込めません | 致命的 | 仕様 |
...
| ARC_009 | データ行なし（空ファイル） | 致命的 | 仕様 |
| ARC_010 | ファイルにUTF-8 BOMあり | 中 | 相互運用 |
...
| STP_015 | level_idがlevels.txtに存在しない | 中 | 仕様 |
...
| TRP_017 | 頻度ベースの便にstop_timesなし | 中 | 仕様 |
| TRP_019 | 連続サービス中にshape_idが必須 | 高 | 仕様 |
| TRP_020 | trip_headsignが中間停留所名と一致 | 低 | 品質 |
| TRP_021 | bikes_allowed（自転車可否）が未設定 | 情報 | 品質 |
| TRP_022 | block内での便時刻の重複 | 高 | 仕様 |
| TRP_023 | 今後7日間にアクティブな便なし | 低 | 品質 |
| TRP_024 | block内での路線タイプの不一致 | 低 | 相互運用 |
| TRP_025 | 車椅子対応情報が未設定の便の割合が高い | 情報 | 品質 |
| TRP_026 | 有効なサービス日がない便 | 中 | 分析 |
...
| ルール | タイトル | 重要度 | クラス |
|---|---|---|---|
| PDW_006 | 同一便・ゾーンで乗降ウィンドウが重複 | 中 | 仕様 |
...
| CAL_003 | start_dateが無効 | 致命的 | 仕様 |
| CAL_004 | end_dateが無効 | 致命的 | 仕様 |
| CAL_005 | start_dateがend_dateより後 | 致命的 | 仕様 |
| CAL_006 | サービス期間にアクティブな曜日なし | 高 | 品質 |
| CAL_007 | サービス期間に空白あり | 中 | 分析 |
| CAL_008 | サービス期間が間もなく終了 | 高 | 分析 |
| CAL_009 | すべての便にアクティブな日付なし | 致命的 | 相互運用 |
| CAL_010 | アクティブ日数が少なすぎる | 中 | 分析 |
| CAL_011 | 使用されていないサービス | 低 | 品質 |
| CAL_012 | 近い将来にアクティブな日付なし | 高 | 分析 |
...
| CLD_002 | 日付の形式が無効 | 致命的 | 仕様 |
| CLD_003 | exception_typeが不足または無効 | 致命的 | 仕様 |
| CLD_004 | 同日に矛盾する例外レコード | 高 | 相互運用 |
| CLD_005 | 日付が範囲外 | 致命的 | 仕様 |
...
| TRF_012 | 乗換レコードが重複 | 中 | 品質 |
| TRF_013 | 乗換タイプがコンテキストと不一致 | 致命的 | 仕様 |
...
| ルール | タイトル | 重要度 | クラス |
|---|---|---|---|
| FPD_001 | fare_product_idが重複 | 致命的 | 仕様 |
...
| ルール | タイトル | 重要度 | クラス |
|---|---|---|---|
| XFL_001 | service_idがcalendar.txtまたはcalendar_dates.txtに存在しない | 致命的 | 仕様 |
| XFL_002 | 便にstop_timesレコードなし | 高 | 仕様 |
| XFL_003 | shape_idが未定義 | 高 | 仕様 |
| XFL_004 | 運賃ルールのroute_idが未定義 | 致命的 | 仕様 |
| XFL_005 | stop_timesのstop_idが未定義 | 致命的 | 仕様 |
| XFL_006 | サービスにキャンセル例外のみ（アクティブ日なし） | 中 | 仕様 |
| XFL_007 | agency_idが存在しない | 致命的 | 仕様 |
| XFL_009 | level_idが無効 | 致命的 | 仕様 |
| XFL_010 | trip_idがtrips.txtに存在しない | 致命的 | 仕様 |
| XFL_011 | カレンダー日付がfeed_info有効期間と不整合 | 中 | 相互運用 |
| XFL_012 | 路線が使用されていない | 高 | 品質 |
| XFL_013 | shape_idが複数の方向で使用されている | 高 | 相互運用 |
...
| OPR_005 | 路線の平均運行間隔 | 情報 | 分析 |
| OPR_006 | 便の停留所数が少なすぎる | 高 | 分析 |
| OPR_007 | 便内に重複する停留所 | 中 | 分析 |
...
| OPR_018 | サービス期間が非常に短い | 中 | 分析 |
| OPR_019 | 路線で同日に複数のサービスが重複 | 情報 | 分析 |
| OPR_020 | 路線の例外日の重複 | 高 | 分析 |
...
| ルール | タイトル | 重要度 | クラス |
|---|---|---|---|
| DQ_001 | フィード名が不足 | 低 | 品質 |
| DQ_002 | feed_publisher_urlが不足 | 低 | 品質 |
| DQ_003 | 路線の説明が不足 | 情報 | 品質 |
| DQ_004 | 路線URLが不足 | 低 | 品質 |
| DQ_005 | 有効なサービス期間なし | 高 | 相互運用 |
| DQ_005b | stop_timesのある便なし | 高 | 相互運用 |
| DQ_005c | 座標なしの停留所の割合が高い | 高 | 相互運用 |
...
| VAT_004 | 路線のサービス非対称（平日のみ運行） | 情報 | 分析 |
