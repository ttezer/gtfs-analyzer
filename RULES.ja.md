...
| ARC_006 | オプションのGTFSファイルが存在 | 情報 | 分析 |
...
| ARC_009 | 任意ファイルにデータ行なし | 情報 | 品質 |
| ARC_010 | ファイルにUTF-8 BOMあり | 情報 | 品質 |
...
| ARC_022 | ファイルの行数が上限（100万行）を超過 | 情報 | 分析 |
...
| ARC_034 | ヘッダー行がデータ行として繰り返されている | 高 | 仕様 |
| ARC_035 | 必須ファイルが空 | 致命的 | 仕様 |
| ARC_036 | ZIPストリームヘッダーが中央ディレクトリと矛盾 | 低 | 品質 |
...
| STM_061 | 連続しない離れた停留所間の速度が不可能 | 高 | 相互運用 |
...
| STM_045 | 便の出発時刻がサービス日の範囲を超過 | 情報 | 分析 |
...
| LOC_012 | locations.geojsonが有効なGeoJSON FeatureCollectionではない | 高 | 仕様 |
...
| CAL_007 | サービス期間に空白あり | 情報 | 分析 |
| CAL_008 | サービス期間が間もなく終了 | 高 | 分析 |
| CAL_009 | フィードの全カレンダー期間が終了 | 情報 | 分析 |
...
| CAL_015 | フィードの最初のサービス日が将来（本日アクティブな便なし） | 情報 | 分析 |
| CAL_016 | サービスが2年以上先まで続く | 情報 | 品質 |
| CAL_017 | カレンダーがまだ開始していない（すべてのアクティブ日が将来） | 情報 | 分析 |
| CAL_018 | アクティブな曜日なし（全曜日0、calendar_dates上書きなし） | 情報 | 品質 |
...
| CAL_024 | 今後7日間にアクティブでないカレンダー | 情報 | 分析 |
...
| SHP_021 | shape_dist_traveledが負または数値でない | 低 | 仕様 |
...
| JPN_016 | GTFS-JP：pattern_jp／旧版 routes_jp の route_update_date が無効 | 中 | 品質 |
...
| JPN_022 | GTFS-JP v4：必須コアフィールドが不足 | 中 | 相互運用 |
| JPN_023 | GTFS-JP：feed_langがjaではない | 高 | 相互運用 |
| JPN_024 | GTFS-JP：agency_langがjaではない | 高 | 相互運用 |
| JPN_025 | GTFS-JP：agency_timezoneがAsia/Tokyoではない | 高 | 相互運用 |
| JPN_026 | GTFS-JP：運賃通貨がJPYではない | 高 | 相互運用 |
| JPN_027 | GTFS-JP V3：route_type が 3 ではない | 高 | 相互運用 |
| JPN_028 | GTFS-JP v3：必須かな翻訳が不足 | 中 | 品質 |
| JPN_029 | GTFS-JP v4：推奨かな翻訳が不足 | 低 | 品質 |
| JPN_030 | GTFS-JP v3：必須日本語翻訳が不足 | 中 | 品質 |
| JPN_031 | GTFS-JP：ゾーン運賃の対象停留所に zone_id がない | 高 | 相互運用 |
| JPN_032 | GTFS-JP V3：agency_idが法人番号形式ではない | 中 | 相互運用 |
| JPN_033 | GTFS-JP：カスタムファイルまたはフィールドで予約済みjp名前空間を使用 | 中 | 相互運用 |
