    ...
    for (row_idx, row) in file.rows.iter().enumerate() {
        ...
        if let Some(code) = &stop_code {
            if let Some(first_stop) = stop_code_owner.get(code) {
                crate::notice_budget::push(&mut notices, make_k2_notice(&mut counter, "STP_039", EntityType::Stop, entity_id.clone(),
        ...
        if stop_code.is_none() && is_stop_or_station {
            crate::notice_budget::push(&mut stp022_pending, make_k2_notice(
        ...
        if zone_id.is_none() && loc_type == 0 {
            crate::notice_budget::push(&mut stp033_pending, make_k2_notice(
