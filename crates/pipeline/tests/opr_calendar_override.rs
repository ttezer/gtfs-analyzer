...
fn opr_single_service_no_conflict_no_notice() {
    ...
    let stop_times: &[u8] = b"trip_id,arrival_time,departure_time,stop_id,stop_sequence\n\
    ...
    assert!(
        !has_rule(&result, "OPR_019"),
        "Tek servis → OPR_019 olmamalı"
    );
    assert!(
        !has_rule(&result, "OPR_020"),
        "Tek servis → OPR_020 olmamalı"
    );
    assert!(
        !has_rule(&result, "OPR_021"),
        "Config yok → OPR_021 olmamalı"
    );
    assert!(
        !has_rule(&result, "OPR_022"),
        "Config yok → OPR_022 olmamalı"
    );
    assert!(
        !has_rule(&result, "OPR_023"),
        "Config yok → OPR_023 olmamalı"
    );
}
...
fn opr_021_absent_when_correct_override() {
    ...
    assert!(
        !has_rule(&result, "OPR_021"),
        "Doğru override → OPR_021 olmamalı"
    );
    assert!(
        !has_rule(&result, "OPR_022"),
        "Doğru override → OPR_022 olmamalı"
    );
    assert!(
        !has_rule(&result, "OPR_023"),
        "Doğru override → OPR_023 olmamalı"
    );
}
