...
//! önbellekli yol `validate_k2_with_whitespace_roots`'a `Some(config.max_file_rows)`
...
fn wasm_never_passes_a_row_budget_to_k2() {
    ...
    while let Some(i) = rest.find("validate_k2_with_whitespace_roots(") {
        let after = &rest[i..];
        ...
        assert!(
            args.len() >= 6,
            "#185: whitespace-root sinyal taşıyan K2 çağrısı altı argümanlı olmalı: {block}"
        );
        assert_eq!(
            args[3], "None",
