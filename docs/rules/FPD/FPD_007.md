...
```
amount sayı olarak ayrıştırılabiliyor  VE  currency üç harfli
  VE  ondalık basamak sayısı != iso4217_minor_unit(currency)
→ sayaç +1  (dosya sonunda tek notice)
```

Sayı olmayan ya da boş tutar **sessiz** geçer — o ihlal `FPD_002`'nin alanıdır ve iki kez
raporlanmamalıdır.

**Minor unit tablosu** (`common.rs::iso4217_minor_unit`) üretilmiş ISO 4217 tablosudur.
Geçersiz/bilinmeyen para birimi kodu **2** varsayılmaz; o durum `FPD_003`'ün alanıdır.
Tabloda minor unit'i tanımsız olan kodlar (kaynakta `255`, ör. fonlar veya kıymetli
madenler) için FPD_007 karşılaştırma yapmaz ve sessiz geçer.
...
- **Yanlış negatif:** Minor unit'i tanımsız olan kodlar bu kuralda karşılaştırılmaz; geçersiz
  para birimi kodları ise `FPD_003` tarafından raporlanır.

## Karışan komşular (ayrım)

| Kural | Ne yakalar | Eksen |
|---|---|---|
| **FPD_007** | ondalık basamak sayısı yanlış | biçim |
| **FPD_002** | tutar eksik / sayı değil / negatif | değer geçerliliği |
| **FPD_003** | para birimi kodu ISO 4217 değil | kod geçerliliği |

## Mesaj & çözüm önerisi
### R9 Kural Mesajı
- **Kural mesajı:** amount para biriminin ISO 4217 ondalık basamak sayısını taşımıyor
- **Çözüm önerisi:** Tutarları para biriminin ondalık basamak sayısıyla yazın (ör. EUR için
  `0.90`, JPY için `150`).

### R2 mesajı/mesajları
- *"fare_products.txt dosyasında N tutar, para biriminin ISO 4217 ondalık basamak sayısını taşımıyor
  (ör. satır 12: EUR 0.9 — 2 basamak beklenir)."*

## Ölçülen alan / değer
- `field`: `amount` · `observed`: etkilenen satır sayısı · `expected`: beklenen basamak

## Kod değiştirirken minimum test matrisi
| Senaryo | Beklenen |
|---|---|
| `EUR 2.50` · `JPY 150` · `KWD 1.500` | sessiz |
| `EUR 0.9` · `EUR 8` · `HKD 6.7` | `FPD_007` |
| `JPY 100.00` | `FPD_007` (yen ondalık taşımamalı) |
| `HKD 0.0000` | `FPD_007` (fazla basamak) |
| boş / `abc` | sessiz (`FPD_002`) |
...
### GTFS spec referansı
- GTFS Schedule Reference, `fare_products.txt` → `amount`
