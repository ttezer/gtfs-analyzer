...
| V3 otobüs `route_type=3` | V3 nihai belge routes.txt; MLIT V3→V4 revizyon açıklaması | V3'ün zaten otobüs formatı olduğu gözden kaçırılıp JPN_027 çıkarılmıştı | **Önceki karar düzeltildi:** JPN_027 açık V3'te ve feed içinde `route_type=3` otobüs ankrajı varken sayısal `route_type != 3` hatları için çalışır; saf non-bus feed'lerde susar. Auto/V4 sessiz |
