...
Rozetin makineyle denetlenebilir hüküm envanteri ve UI sözleşmesi için [kapsama belgesine](gtfs-jp-automated-coverage.md) bakın.
...
| Japonca metin tespiti | Katalog ölçümü: 4.311 feed | Kanji tek başına kullanılırsa Çin/Tayvan feed'leri de açılıyordu; stop sinyali üretim akışında erken hesaplanıyordu | Ortak tespit kapısı `agency_name` veya `stop_name` içinde en az bir kana kullanır; tespit `stops.txt` parse edildikten sonra hesaplanır. Beş kaçan JP feed'inin tamamı `agency_lang=ja` + `Asia/Tokyo` ile yakalanır; ölçülen negatiflerde yanlış pozitif çıkmadı |
...
| `routes_jp.txt` | V3 standardından kaldırıldı; parser/sinyal ve legacy JPN_015/JPN_016 yalnız Auto profilinde korunur | V4 ana standardında yok | Legacy uyumluluk | auto: JPN_015, JPN_016; v3/v4: JPN_033 ad alanı kontrolü | Interop / Quality | [v3 nihai belge](https://www.mlit.go.jp/sogoseisaku/transport/content/001981046.docx) / v4 farkı | Profil başına eski dosyanın sonucu |
...
- Varsayılan `auto` profilinde mevcut legacy/v3 davranışı korunur; buna `routes_jp.txt` için JPN_015/JPN_016 uyumluluk kontrolleri de dahildir. Açık `v3` profilinde yalnız V3 standardında kalan uzantı kuralları çalışır; `routes_jp.txt` v3 dışı ad olarak JPN_033'e tabidir. `v4` profilinde `agency_jp.txt`, `office_jp.txt` ve `pattern_jp.txt` referans verisi olarak okunabilir ama ilgili v3 bulguları üretilmez; `routes_jp.txt` de JPN_033 kapsamında özel ad alanı olarak değerlendirilir. Bu seçim feed'in sürümünü otomatik kanıtlamaz.
