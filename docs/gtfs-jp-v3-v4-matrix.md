...
| `routes_jp.txt` | V3 standardından kaldırıldı; parser/sinyal ve legacy JPN_015/JPN_016 V3 ve Auto profillerinde korunur | V4 ana standardında yok; bilinen legacy dosya olarak sessizce okunur | Legacy uyumluluk | v3/auto: JPN_015, JPN_016; v4: legacy dosya, JPN_033 muafı | Interop / Quality | [v3 nihai belge](https://www.mlit.go.jp/sogoseisaku/transport/content/001981046.docx) / v4 farkı | Profil başına eski dosyanın sonucu |
...
- Varsayılan `auto` profilinde mevcut legacy/v3 davranışı korunur; buna `routes_jp.txt` için JPN_015/JPN_016 uyumluluk kontrolleri de dahildir. Açık `v3` profilinde de bu legacy kontrolleri korunur; dosya normatif V3 envanterinde sayılmaz. `v4` profilinde `agency_jp.txt`, `office_jp.txt`, `routes_jp.txt` ve `pattern_jp.txt` bilinen eski dosyalar olarak okunabilir ama ilgili v3 bulguları üretilmez. Bu seçim feed'in sürümünü otomatik kanıtlamaz.
