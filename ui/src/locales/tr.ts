const tr = {
  ui: {
    // Nav
    'nav.report' : 'Rapor',
    'nav.fix'    : 'Ayrıntı ve Düzeltme',
    'nav.files'  : 'GTFS Dosyaları',
    'nav.fileMap': 'Dosya Haritası',
    'nav.export' : 'Dışa Aktar',
    'nav.compare': 'Karşılaştır',
    'compare.title': 'Sürüm Karşılaştırması',
    'compare.subtitle': 'Az önce analiz edilen feed’i eski bir Golden JSON ile karşılaştırın.',
    'compare.select_old': "Eski Golden JSON'u Seç",
    'compare.empty_title': 'Karşılaştırmaya hazır',
    'compare.empty_desc': 'Eski analizden dışa aktarılan Golden JSON dosyasını yükleyin.',
    'compare.before': 'Karşılaştırılan', 'compare.now': 'Mevcut', 'compare.unknown_date': 'tarih bilinmiyor',
    'compare.pub_score': 'Yayın Skoru', 'compare.overall_score': 'Genel Skor', 'compare.total_notice': 'Toplam Notice',
    'compare.scores': 'Skor Karşılaştırması', 'compare.rule_changes': 'Kural Değişimleri',
    'compare.distribution': 'Notice Dağılımındaki Değişim', 'compare.by_severity': 'Önem seviyesine göre', 'compare.by_class': 'Analiz sınıfına göre',
    'compare.all': 'Tümü', 'compare.fixed': 'Düzeltildi', 'compare.decreased': 'Azaldı',
    'compare.increased': 'Arttı', 'compare.new': 'Yeni', 'compare.same': 'Aynı',
    'compare.search': 'Kural, kod veya açıklama ara…', 'compare.status': 'Durum', 'compare.rule': 'Kural',
    'compare.description': 'Açıklama', 'compare.delta': 'Fark', 'compare.change': 'Değişim',
    'compare.no_metadata': 'Eski Golden dosyada kural açıklaması yok', 'compare.no_rows': 'Bu filtrede değişiklik yok.',
    'compare.feed_structure': 'Feed Yapısındaki Değişim',
    'compare.feed_range': 'Feed geçerlilik aralığı', 'compare.service_range': 'Gerçek servis aralığı',
    'compare.per_1000_trips': '1.000 trip başına notice', 'compare.per_100k_stop_times': '100.000 stop_time başına notice',
    'compare.feed_note': 'Notice değişimleri feed boyutundan ve servis tarih aralığından etkilenebilir.',
    'compare.feed_warning': 'Dosya adları farklı; aynı feed olduğundan emin olun',
    'compare.date_warning': 'Feed veya servis tarih aralığı değişmiş. Tarihe bağlı kural farklarını düzeltme olarak yorumlamadan önce bu değişimi dikkate alın.',
    'compare.config_warning': 'İki analiz farklı kriter ayarlarıyla çalıştırılmış. Bazı farklar veri değişikliğinden değil eşik değişikliğinden kaynaklanabilir.',
    'compare.legacy_warning': 'Eski Golden v1–v3 dosyası: zaman, dosya satırları ve kural metadata’sı sınırlı olabilir.',
    'compare.legacy_class_note': 'Eski Golden sürümleri sınıf kırılımı içermez; sınıf tablosunda “Karşılaştırılan” sütunu “—” gösterilir.',
    'compare.download_csv': 'Karşılaştırma CSV’sini İndir',
    'compare.score.pub_score': 'Yayın', 'compare.score.overall': 'Genel', 'compare.score.spec': 'Spec',
    'compare.score.interop': 'Interop', 'compare.score.quality': 'Quality', 'compare.score.analytics': 'Analytics',
    'fileMap.title': 'GTFS Dosya Haritası',
    'fileMap.subtitle': 'Dosyaları, ilişkileri ve analyzer bulgularını birlikte inceleyin.',
    'fileMap.fileFilter': 'Dosyalar',
    'fileMap.allFiles': 'Tüm dosyalar',
    'fileMap.presentFiles': 'Yalnız mevcut',
    'fileMap.severityFilter': 'Önem',
    'fileMap.allSeverities': 'Tüm önemler',
    'fileMap.selectFile': 'Dosyaya git',
    'fileMap.coreView': 'Çekirdek GTFS görünümü',
    'fileMap.zoomControls': 'Harita yakınlaştırma kontrolleri',
    'fileMap.zoomIn': 'Yakınlaştır',
    'fileMap.zoomOut': 'Uzaklaştır',
    'fileMap.fit': 'Ekrana sığdır',
    'fileMap.canvasLabel': 'GTFS dosyaları ve referans ilişkileri',
    'fileMap.clean': 'Temiz',
    'fileMap.missing': 'Eksik',
    'fileMap.hasFindings': 'Bulgu var',
    'fileMap.notPresent': 'Feed içinde bulunmuyor',
    'fileMap.file': 'GTFS dosyası',
    'fileMap.rows': 'satır',
    'fileMap.findings': 'bulgu',
    'fileMap.specReference': 'GTFS referansı',
    'fileMap.openSpec': 'Resmi GTFS referansını yeni sekmede aç',
    'fileMap.backToCore': 'GTFS Görünümüne Dön',
    'fileMap.overview': 'Genel görünüm',
    'fileMap.coreFiles': 'Standart GTFS Dosyaları',
    'fileMap.coreIntro': 'GTFS feed yapısının çekirdek dosyaları. İlişkileri keşfetmek için bir dosya seçin.',
    'fileMap.scopeNote': '7 çekirdek dosyanın dışında yalnızca analyzer bulgusu bulunan dosyalar gösterilir.',
    'fileMap.present': 'Mevcut',
    'fileMap.coreHint': 'Bir dosyaya tıkladığınızda yalnızca doğrudan ilişkili dosyalar ve bağlantılar açılır.',
    'fileMap.openFindings': 'Tüm Bulguları Aç',
    'fileMap.rules': 'Kurala göre bulgular',
    'fileMap.noFindings': 'Bu dosya için gösterilen bulgu yok.',
    'fileMap.relationship': 'Dosya ilişkisi',
    'fileMap.relationshipNote': 'Bu ilk prototipte kenarlar GTFS şemasını gösterir; kenar bazlı hata sayısı henüz hesaplanmaz.',
    'fileMap.group.core': 'Ana Servis',
    'fileMap.group.calendar': 'Takvim',
    'fileMap.group.geometry': 'Geometri',
    'fileMap.group.station': 'İstasyon Erişimi',
    'fileMap.group.fare-v1': 'Ücret V1',
    'fileMap.group.fare-v2': 'Ücret V2',
    'fileMap.group.network': 'Alanlar ve Ağlar',
    'fileMap.group.flex': 'Flex',
    'fileMap.group.support': 'Destekleyici Dosyalar',
    'fileMap.group.external': 'Spec Dışı Dosyalar',
    'fileMap.nonSpec': 'Spec dışı',
    'fileMap.nonSpecPresent': 'Spec dışı dosya feed içinde mevcut',
    // Header
    'header.title' : 'GTFS Validator & Analyzer',
    'header.back'  : '← Ana Sayfa',
    'header.new'   : 'Yeni GTFS Yükle',
    'lang.switch'  : 'EN',
    'theme.light'  : 'Açık mod',
    'theme.dark'   : 'Koyu mod',
    // Upload
    'upload.files_section'    : 'GTFS Dosyaları',
    'upload.stages_section'   : 'İşlem Aşamaları',
    'upload.no_files'         : 'Henüz dosya yüklenmedi',
    'upload.drag'             : 'ZIP sürükleyin',
    'upload.drop_aria'        : 'ZIP dosyası yükle',
    'upload.file_input_aria'  : 'ZIP dosyası seç',
    'upload.unit_sec'         : 'sn',
    'upload.unit_ms'          : 'ms',
    'upload.load'             : 'GTFS Yükle',
    'upload.load_another'     : 'Farklı GTFS Yükle',
    'upload.show_report'      : 'Raporu Göster →',
    'upload.report_ready'     : 'Rapor {duration} sn’de hazırlandı',
    'upload.waiting'          : 'bekliyor',
    'upload.running'          : 'çalışıyor…',
    'upload.row_unit'         : 'satır',
    'upload.loaded_file'      : 'Yüklenen dosya:',
    'engine.wasm64-serial'    : 'WASM64 · seri',
    'engine.wasm32-threaded'  : 'WASM32 · paralel',
    'engine.wasm32-serial'    : 'WASM32 · seri',
    'upload.settings_title'   : 'Analiz Kriterleri',
    'upload.settings_modified': 'Değiştirildi',
    'upload.apply'            : 'Uygula',
    'upload.reset'            : 'Sıfırla',
    'upload.settings_saved'   : 'Ayarlar kaydedildi. Yeni ZIP yükleyince uygulanır.',
    'upload.settings_reset'   : 'Varsayılanlara döndürüldü.',
    'upload.error_zip'        : 'Yalnızca .zip uzantılı dosyalar kabul edilir.',
    'upload.error_size'       : 'Dosya çok büyük ({mb} MB). Maksimum izin verilen boyut: 1024 MB.',
    'upload.url_placeholder'  : 'https://…/gtfs.zip',
    'upload.url_aria'         : 'Feed URL’si',
    'upload.url_load'         : 'URL’den Yükle',
    'upload.url_downloading'  : '{name} indiriliyor…',
    'upload.url_hint'         : 'CORS izni olan kaynaklarda çalışır (ör. Mobility Database); diğerlerinde ZIP’i indirip sürükleyin.',
    'upload.url_invalid'      : 'Geçersiz URL.',
    'upload.url_https'        : 'Yalnızca https URL’leri desteklenir.',
    'upload.url_credentials'  : 'URL kullanıcı adı/parola içeremez.',
    'upload.url_timeout'      : 'İstek zaman aşımına uğradı (30 sn).',
    'upload.url_http'         : 'Sunucu {status} yanıtı döndü.',
    'upload.url_body'         : 'Dosya indirilirken bağlantı kesildi.',
    'upload.url_not_zip'      : 'İndirilen içerik bir ZIP dosyası değil.',
    'upload.url_blocked'      : 'Tarayıcı bu adresteki dosyayı okuyamadı — kaynak erişilemiyor olabilir veya sunucu tarayıcı erişimine (CORS) izin vermiyor olabilir.',
    'upload.url_open_link'    : 'Bağlantıyı aç',
    'upload.url_then_drop'    : '— sonra indirdiğiniz ZIP’i buraya sürükleyin.',
    'upload.default_for'      : 'varsayılan: {val}',
    // Stages
    'stage.K1' : 'K1 — Dosya Ayrıştırma',
    'stage.K2' : 'K2 — Kural Doğrulama',
    'stage.K3' : 'K3 — Varlık Haritası',
    'stage.K4' : 'K4 — Çapraz Kontrol',
    'stage.K5' : 'K5 — Türetilmiş Veri',
    'stage.K6' : 'K6 — İstatistik Analizi',
    'stage.K7' : 'K7 — Rapor Oluşturma',
    // File row labels
    'file.stops.txt'         : 'durak',
    'file.routes.txt'        : 'hat',
    'file.trips.txt'         : 'sefer',
    'file.stop_times.txt'    : 'geçiş zamanı',
    'file.shapes.txt'        : 'güzergah',
    'file.agency.txt'        : 'işletici',
    'file.calendar.txt'      : 'takvim',
    'file.calendar_dates.txt': 'özel gün',
    'file.frequencies.txt'   : 'frekans',
    'file.transfers.txt'     : 'aktarma',
    'file.pathways.txt'      : 'geçit',
    'file.levels.txt'        : 'kat',
    'file.attributions.txt'  : 'atıf',
    'file.translations.txt'  : 'çeviri',
    // Config fields
    'cfg.gtfs_jp_profile.label' : 'GTFS-JP profil kapsamı',
    'cfg.gtfs_jp_profile.desc'  : 'Sürüm otomatik tespit edilmez; Auto legacy davranışı korur, V3/V4 açık kapsam seçer.',
    'cfg.gtfs_jp_profile.auto'  : 'Auto (legacy)',
    'cfg.gtfs_jp_profile.v3'    : 'V3',
    'cfg.gtfs_jp_profile.v4'    : 'V4',
    'cfg.max_speed_bus_kmh.label'        : 'Maks. Otobüs Hızı',
    'cfg.max_speed_bus_kmh.unit'         : 'km/s',
    'cfg.max_speed_bus_kmh.desc'         : 'Otobüs seferleri için maksimum izin verilen hız',
    'cfg.max_speed_tram_kmh.label'       : 'Maks. Tramvay Hızı',
    'cfg.max_speed_tram_kmh.unit'        : 'km/s',
    'cfg.max_speed_tram_kmh.desc'        : 'Tramvay seferleri için maksimum izin verilen hız',
    'cfg.max_speed_metro_kmh.label'      : 'Maks. Metro Hızı',
    'cfg.max_speed_metro_kmh.unit'       : 'km/s',
    'cfg.max_speed_metro_kmh.desc'       : 'Metro seferleri için maksimum izin verilen hız',
    'cfg.max_speed_rail_kmh.label'       : 'Maks. Demiryolu Hızı',
    'cfg.max_speed_rail_kmh.unit'        : 'km/s',
    'cfg.max_speed_rail_kmh.desc'        : 'Demiryolu seferleri için maksimum izin verilen hız',
    'cfg.max_speed_ferry_kmh.label'      : 'Maks. Feribot Hızı',
    'cfg.max_speed_ferry_kmh.unit'       : 'km/s',
    'cfg.max_speed_ferry_kmh.desc'       : 'Feribot seferleri için maksimum izin verilen hız',
    'cfg.max_speed_cablecar_kmh.label'   : 'Maks. Teleferik Hızı',
    'cfg.max_speed_cablecar_kmh.unit'    : 'km/s',
    'cfg.max_speed_cablecar_kmh.desc'    : 'Teleferik/füniküler için maksimum izin verilen hız',
    'cfg.min_transfer_time_sec.label'    : 'Min. Aktarma Süresi',
    'cfg.min_transfer_time_sec.unit'     : 'sn',
    'cfg.min_transfer_time_sec.desc'     : 'Transferler için minimum bağlantı süresi',
    'cfg.max_transfer_distance_m.label'  : 'Maks. Aktarma Mesafesi',
    'cfg.max_transfer_distance_m.unit'   : 'm',
    'cfg.max_transfer_distance_m.desc'   : 'Transfer geçerli sayılmak için maksimum mesafe',
    'cfg.max_shape_jump_km.label'        : 'Maks. Güzergah Sıçraması',
    'cfg.max_shape_jump_km.unit'         : 'km',
    'cfg.max_shape_jump_km.desc'         : 'Art arda güzergah noktaları arasındaki maksimum mesafe',
    'cfg.stop_too_close_m.label'         : 'Çok Yakın Durak Eşiği',
    'cfg.stop_too_close_m.unit'          : 'm',
    'cfg.stop_too_close_m.desc'          : 'Bu mesafeden yakın duraklar tekrar sayılır',
    'cfg.stop_far_from_shape_m.label'    : "Durağın Güzergah'a Uzaklığı",
    'cfg.stop_far_from_shape_m.unit'     : 'm',
    'cfg.stop_far_from_shape_m.desc'     : 'Durağın güzergahından en fazla bu kadar uzakta olabilir',
    'cfg.stop_far_from_parent_m.label'   : 'Üst İstasyona Uzaklık Eşiği',
    'cfg.stop_far_from_parent_m.unit'    : 'm',
    'cfg.stop_far_from_parent_m.desc'    : 'Durak, üst istasyonundan en fazla bu kadar uzakta olabilir',
    'cfg.feed_expiry_warning_days.label' : 'Son Kullanma Uyarısı',
    'cfg.feed_expiry_warning_days.unit'  : 'gün',
    'cfg.feed_expiry_warning_days.desc'  : 'Feed bu kadar günden az kalmışsa uyarı üretilir',
    'cfg.feed_info_expiry_warning_days.label' : 'Feed Bilgisi Son Kullanma Uyarısı',
    'cfg.feed_info_expiry_warning_days.unit'  : 'gün',
    'cfg.feed_info_expiry_warning_days.desc'  : 'FIN_019 için feed_end_date bu kadar gün içinde bitiyorsa uyarı üretilir; CAL_008 eşiğinden ayrıdır',
    'cfg.service_gap_days.label'         : 'Servis Boşluğu Eşiği',
    'cfg.service_gap_days.unit'          : 'gün',
    'cfg.service_gap_days.desc'          : 'Bu günden uzun servis kesintisi işaretlenir',
    'cfg.big_gap_days.label'            : 'Büyük Servis Boşluğu',
    'cfg.big_gap_days.unit'             : 'gün',
    'cfg.big_gap_days.desc'             : 'Bir servisin bu kadar gün veya daha uzun kesintisi işaretlenir (CAL_007/012)',
    'cfg.upcoming_service_days.label'    : 'Yakın Servis Penceresi',
    'cfg.upcoming_service_days.unit'     : 'gün',
    'cfg.upcoming_service_days.desc'     : 'Bugünü kapsayan ama bu kadar gün içinde seferi olmayan servisleri işaretler (CAL_021)',
    'cfg.max_trip_duration_hours.label'  : 'Maks. Sefer Süresi',
    'cfg.max_trip_duration_hours.unit'   : 'saat',
    'cfg.max_trip_duration_hours.desc'   : 'Tek bir seferin maksimum süresi',
    'cfg.min_trip_duration_sec.label'    : 'Min. Sefer Süresi',
    'cfg.min_trip_duration_sec.unit'     : 'sn',
    'cfg.min_trip_duration_sec.desc'     : 'Tek bir seferin minimum süresi',
    'cfg.max_headway_warning_min.label'  : 'Maks. Sefer Aralığı',
    'cfg.max_headway_warning_min.unit'   : 'dk',
    'cfg.max_headway_warning_min.desc'   : 'Bu dakikadan uzun aralık uyarı üretir',
    'cfg.bunching_threshold_min.label'   : 'Sıkışma Eşiği',
    'cfg.bunching_threshold_min.unit'    : 'dk',
    'cfg.bunching_threshold_min.desc'    : 'Bu dakikadan kısa aralık sıkışma sayılır',
    'cfg.service_day_start_hour.label'   : 'Servis Günü Başlangıcı',
    'cfg.service_day_start_hour.unit'    : ':00',
    'cfg.service_day_start_hour.desc'    : 'Bir seferde bu saatten önceki zamanlar gece yarısı sonrası servis kabul edilir (00:xx → 24:xx); yanlış "zaman geriye gidiyor" hatalarını önler. 0 = kapalı.',
    'cfg.max_calendar_future_years.label': 'Maks. Takvim Geleceği',
    'cfg.max_calendar_future_years.unit' : 'yıl',
    'cfg.max_calendar_future_years.desc' : 'Bir takvim end_date bu yıldan fazla ileriyse şüpheli/düşük-kaliteli veri işaretlenir (ör. 2050/2099 yer-tutucu).',
    // Score panel
    'score.pub'  : 'Yayın Skoru',
    'score.overall' : 'Genel Skor',
    // Class labels
    'class.SPEC'      : 'GTFS Geçerliliği',
    'class.INTEROP'   : 'GTFS Uyumluluğu',
    'class.QUALITY'   : 'GTFS Kalitesi',
    'class.ANALYTICS' : 'GTFS Analitiği',
    // Domain
    'domain.pub_score'           : 'Yayın Skoru',
    'domain.pub_hint'            : 'SPEC + INTEROP blocker temizliği',
    'domain.r1_incomplete_title': 'Yayın kararı verilemedi',
    'domain.r1_incomplete_desc': 'Zorunlu bir dosya okunamadı; {n} kontrol hiç çalıştırılamadı. Bulgu görünmemesi feed’in temiz olduğu anlamına GELMEZ.',
    'domain.score_incomplete_warn': 'Bu skor EKSİK KAPSAMLI bir doğrulamadan geldi ve başka feed’lerle kıyaslanamaz: okunamayan dosyanın kuralları hiç çalışmadığı için ceza üretemez ve skor olduğundan yüksek çıkar.',
    'domain.overall_score'       : 'Genel Skor',
    'domain.overall_hint'        : 'Tüm sınıfların ağırlıklı ortalaması (Spec×40% + Interop×30% + Quality×20% + Analytics×10%)',
    'domain.r1_blocked_title'    : 'Yayınlanması Tavsiye Edilmez',
    'domain.r1_blocked_desc'     : '{n} kritik engelleyici bulgu mevcut — feed yayına alınamaz.',
    'domain.r1_ok_title'         : 'Yayına Uygun',
    'domain.r1_ok_desc'          : 'Engelleyici bulgu yok.',
    'domain.subscores_title'     : 'Skor Bileşenleri',
    'domain.subscore_filter_tip' : 'Bu bileşenin bulgularını Ayrıntı ve Düzeltme sayfasında görmek için tıklayın',
    'domain.pie_title'           : 'Hata Dağılımı',
    'domain.pie_center'          : 'hata',
    'domain.sev.CRITICAL'        : 'Kritik',
    'domain.sev.HIGH'            : 'Yüksek',
    'domain.sev.MEDIUM'          : 'Orta',
    'domain.sev.LOW'             : 'Düşük',
    'domain.sev.INFO'            : 'Bilgi',
    'domain.metrics_title'       : 'Feed Metrikleri',
    'domain.gtfs_jp.tip'         : 'GTFS-JP (Japonya profili) feed — feed_lang=ja, kana çevirileri (ja-Hrkt) veya *_jp dosyalarından algılandı.',
    'domain.gtfs_jp.profile'     : 'GTFS-JP {profile} profili',
    'domain.gtfs_jp.profile_tip' : 'Bu rozet analizde seçilen kural kapsamını gösterir; feed’in resmî GTFS-JP sürümü otomatik olarak tespit edilmez.',
    'domain.metric.stops'        : 'Durak',
    'domain.metric.routes'       : 'Hat',
    'domain.metric.trips'        : 'Sefer',
    'domain.metric.shapes'       : 'Güzergah',
    'domain.metric.service_days' : 'Aktif Servis Günü',
    'domain.metric.avg_trips'    : 'Ort. Günlük Sefer',
    'domain.table.file'          : 'Dosya',
    'domain.table.rows'          : 'Satır',
    'domain.table.size'          : 'Boyut',
    // Feed takvimi / geçerlilik
    'domain.calendar_title'      : 'Feed Takvimi',
    'domain.feed_validity'       : 'Geçerlilik penceresi',
    'domain.service_coverage'    : 'Servis kapsaması',
    'domain.active_days_fmt'     : '{span} günün {active} günü aktif',
    'domain.coverage_fmt'        : '%{pct} kapsama',
    'domain.generated_at'        : 'Rapor oluşturuldu',
    'domain.not_specified'       : 'belirtilmemiş',
    'domain.no_service'          : 'Servis tarihi bulunamadı',
    // Fix labels
    'label.blocker'             : 'Engelleyici',
    'label.interop'             : 'Uyumluluk',
    'label.propagation'         : 'Zincirleme',
    'label.quick-win'           : 'Kolay Düzeltme',
    'label.quality'             : 'Kalite Öncelikli',
    'label.widespread'          : 'Yaygın',
    'label.analytics'           : 'Analitik',
    'label.hard'                : 'Zor',
    'label.single'              : 'Tek Örnek',
    'label.high-impact'         : 'Yüksek Etki',
    // Fix R9
    'fix.r9_title'       : 'Kural Bazlı Skor Özeti (R9)',
    'fix.r9_hint'        : 'Satıra tıklayarak açıklama ve çözüm önerisini görebilirsiniz.',
    'fix.r9_collapsed_hint' : 'Yayın ve Genel Skora etkiyi görmek için tıklayın',
    'fix.r9_empty'       : 'Düzeltilecek bulgu yok.',
    'fix.r9_message'     : 'Mesaj:',
    'fix.r9_remediation' : 'Çözüm:',
    'fix.th.rule'          : 'Kural',
    'fix.th.severity'      : 'Önem',
    'fix.th.label'         : 'Etiket',
    'fix.th.count'         : 'Hata',
    'fix.th.count.tip'     : 'Bu kuraldan kaç hata üretildi — sorunun yaygınlığını gösterir.',
    'fix.th.total'         : 'Toplam',
    'fix.th.total.tip'     : 'Kural başına notice sınırı (cap) uygulandığında gerçek ihlal sayısı. Sınıra çarpmayan kurallarda bu sütun boş kalır.',
    'fix.th.score'         : 'Skor',
    'fix.th.score.tip'     : 'Öncelik puanı. Ciddiyet × (1+Bağımlı) × log₂(1+Etkilenen) / Çaba. Yüksek skor = önce düzelt.',
    'fix.th.pub'           : '+Yayın',
    'fix.th.pub.tip'       : 'Bu kural düzeltilirse yayın skoru kaç puan artar. Toplamı = 100 − mevcut yayın skoru.',
    'fix.th.quality'       : '+Skor',
    'fix.th.quality.tip'   : 'Bu kural düzeltilirse genel skor kaç puan artar. Toplamı = 100 − mevcut genel skor.',
    'fix.th.dependent'     : 'Bağımlı',
    'fix.th.dependent.tip' : "Bu kural düzeltilince kaç başka kural daha kendiliğinden kapanır.",
    'fix.th.effort'        : 'Çaba',
    'fix.th.effort.tip'    : 'Düzeltme iş yükü: 1=kolay (tek alan), 2=orta (çapraz-dosya), 3=zor (yapısal/veri modeli).',
    // Fix R2
    'fix.r2_title'          : 'Tüm Bulgular (R2)',
    'fix.r2_empty'          : 'Bulgu yok.',
    'fix.filter.severity'   : 'Önem filtrele:',
    'fix.filter.class'      : 'Sınıf filtrele:',
    'fix.filter.rule'       : 'Kural filtrele:',
    'fix.filter.all'        : 'Tümü',
    'fix.filter.count'      : '{total} noticeden {visible} tanesi',
    'fix.cap_warning'       : "Bu kural cap'e çarptı — raporda yalnızca {shown} notice gösterilmektedir, gerçek toplam: {total}.",
    'fix.r2.th.rule'        : 'Kural',
    'fix.r2.th.severity'    : 'Önem',
    'fix.r2.th.class'       : 'Sınıf',
    'fix.r2.th.message'     : 'Mesaj',
    'fix.r2.th.file'        : 'Dosya',
    'fix.r2.th.service'     : 'Çalışma Takvimi',
    'fix.r2.th.row'         : 'Satır',
    'fix.r2.th.field'       : 'Alan',
    'fix.r2.th.pub'         : '+Yayın',
    'fix.r2.th.pub.tip'     : 'Bu hatayı düzeltmenin yayın skoruna katkısı. Toplamı = 100 − mevcut yayın skoru.',
    'fix.r2.th.quality'     : '+Skor',
    'fix.r2.th.quality.tip' : 'Bu hatayı düzeltmenin genel skora katkısı. Toplamı = 100 − mevcut genel skor.',
    'fix.map_btn'           : 'Haritada göster',
    'fix.pattern_btn'       : 'Sefer desenlerini göster',
    'fix.pattern.title'     : 'Sefer Desenleri',
    'fix.pattern.close'     : 'Kapat',
    'fix.map.title'         : 'Harita',
    'fix.map.close'         : 'Kapat',
    'fix.pattern.none'      : 'Bu hatta durak dizisi bilinen sefer yok.',
    'fix.pattern.pat_n'     : 'Desen {n}',
    'fix.pattern.trips'     : '{n} sefer',
    'fix.pattern.stops_n'   : '{n} durak',
    'fix.pattern.summary'   : '{p} farklı desen · toplam {t} sefer',
    'fix.pattern.limited'   : 'Güvenlik nedeniyle gösterim sınırlandı; {n} sefer gösterilmedi.',
    'fix.pattern.dir_out'   : 'Gidiş',
    'fix.pattern.dir_in'    : 'Dönüş',
    'fix.spec_link'         : 'GTFS spesifikasyonu (yeni sekme)',
    'fix.map.shape'         : 'şekil',
    // Map legend labels
    'fix.map.route_shape'          : 'Güzergah şekli',
    'fix.map.route_shape_rev'      : 'Güzergah şekli (ters)',
    'fix.map.route_shape_named'    : 'Güzergah şekli — {name}',
    'fix.map.route_shapes_named'   : 'Güzergah şekilleri — {name}',
    'fix.map.route_shape_unused'   : 'Kullanılmayan güzergah şekli',
    'fix.map.route_shape_single'   : 'Tek güzergah şekli',
    'fix.map.neighbor_stops'       : 'Komşu duraklar',
    'fix.map.bad_seq_stop'         : 'Sırası bozuk durak',
    'fix.map.depart_stop'          : 'Kalkış durağı',
    'fix.map.arrive_stop'          : 'Varış durağı',
    'fix.map.depart_arrive_stops'  : 'Kalkış / varış durakları (hat rengine göre)',
    'fix.map.dup_point'            : 'Tekrarlanan nokta',
    'fix.map.repeat_point'         : 'Tekrarlayan nokta',
    'fix.map.ambiguous_stop'       : 'Belirsiz durak',
    'fix.map.stops'                : 'Duraklar',
    'fix.map.stop'                 : 'Durak',
    'fix.map.trip_stops'           : 'Sefer durakları',
    'fix.map.repeated_stop'        : 'Tekrar eden durak',
    'fix.map.bad_segment'          : 'Bozuk segment',
    'fix.map.segment_ends'         : 'Segment uçları',
    'fix.map.impossible_speed'     : 'İmkansız hız segmenti',
    'fix.map.far_segment_end'      : 'Aşırı uzak segment sonu',
    'fix.map.short_segment_end'    : 'Kısa süreli segment sonu',
    'fix.map.single_stop_trip'     : 'Tek durak (kullanılamaz sefer)',
    'fix.map.revisit_stop'         : 'Tekrar ziyaret edilen durak',
    'fix.map.far_endpoint_stop'    : 'Uçtan uzak durak',
    'fix.map.first_stop_wrong_end' : 'İlk durak (yanlış uçta)',
    'fix.map.terminal_stop'        : 'Terminal durağı',
    'fix.map.transfer_hub_stop'    : 'Aktarma merkezi durağı',
    'fix.map.connected_stops'      : 'Bağlı duraklar',
    'fix.map.isolated_stops'       : 'İzole duraklar',
    'fix.map.jump_segment'         : 'Atlayan segment',
    'fix.map.far_stop'             : 'Uzak durak',
    'fix.map.far_stop_from_shape'  : 'Güzergahtan uzak durak',
    'fix.map.pin.far_from_shape'   : 'Güzergahtan uzak',
    'fix.map.degenerate_shape'     : 'Tüm noktalar bu koordinatta',
    'fix.map.null_island_point'    : 'Null Island yakını nokta',
    'fix.map.feed_median'          : 'Feed medianı (merkez)',
    'fix.map.far_from_median'      : 'Medyandan {km} km uzakta',
    'fix.map.jump_start'           : 'Atlama başlangıcı',
    'fix.map.jump_end'             : 'Atlama bitişi',
    'fix.map.segment_1'            : '1. kesişen segment',
    'fix.map.segment_2'            : '2. kesişen segment',
    'fix.map.parent_station_label' : 'Üst İstasyon',
    'fix.map.n_stops'              : '{n} durak',
    // Map pin labels
    'fix.map.pin.prev'             : '(önceki)',
    'fix.map.pin.next'             : '(sonraki)',
    'fix.map.pin.bad_seq'          : '⚠ sıra bozuk',
    'fix.map.pin.ambiguous'        : '⚠ belirsiz eşleşme',
    'fix.map.pin.depart'           : '(kalkış)',
    'fix.map.pin.arrive'           : '(varış)',
    'fix.map.pin.repeated'         : '(tekrar eden)',
    'fix.map.pin.single_stop'      : '(tek durak — sefer kullanılamaz)',
    'fix.map.pin.revisit'          : '(ardışık iki kez ziyaret)',
    'fix.map.pin.parent'           : '(üst istasyon)',
    'fix.map.pin.depart_speed'     : '(kalkış — imkansız hız)',
    'fix.map.pin.arrive_speed'     : '(varış — imkansız hız)',
    'fix.map.pin.arrive_far'       : '(varış — çok uzak)',
    'fix.map.pin.arrive_short'     : '(varış — çok kısa süre)',
    'fix.map.pin.first_wrong'      : "⚠ ilk durak — shape'in yanlış ucuna düşüyor",
    'fix.map.pin.first_far_start'  : '⚠ ilk durak — shape başından uzak',
    'fix.map.pin.last_far_end'     : '⚠ son durak — shape bitişinden uzak',
    'fix.map.pin.dup_coord'        : 'Tekrarlanan koordinat',
    'fix.map.pin.repeat_coord'     : 'Tekrarlayan koordinat',
    'fix.map.pin.jump_start'       : 'Atlama başlangıcı',
    'fix.map.pin.jump_end'         : 'Atlama bitişi',
    'fix.map.pin.seg_start'        : 'Segment {a}→{b} başlangıcı',
    'fix.map.pin.seg_end'          : 'Segment {a}→{b} bitişi',
    'fix.map.pin.terminal'         : 'Terminal: {name}',
    'fix.map.pin.station'          : 'İstasyon: {name}',
    'fix.map.pin.isolated'         : '⚠ İzole',
    'fix.map.pin.isolated_routes'  : '{routes} hat • {trips} sefer',
    // Export
    'export.title'                    : 'Dışa Aktar',
    'export.source'                   : 'Kaynak:',
    'export.html'                     : 'HTML Rapor İndir',
    'export.csv'                      : 'CSV İndir',
    'export.json'                     : 'JSON Ham Veri İndir',
    'export.pdf'                      : 'PDF Olarak Yazdır',
    'export.formats.title'            : 'Dışa aktarım formatını seçin',
    'export.formats.subtitle'         : 'Seçtiğiniz formatta raporları ve verileri yerel olarak indirebilirsiniz.',
    'export.recommended'              : 'Önerilen',
    'export.download'                 : 'İndir',
    'export.print'                    : 'Yazdır',
    'export.est_size'                 : 'Tahmini boyut:',
    'export.size.browser'             : 'Tarayıcı çıktısı',
    'export.card.html.title'          : 'HTML Rapor',
    'export.card.html.desc'           : 'Her bulgu için dosya, satır, alan, gözlenen/beklenen değer ve düzeltme önerisini içeren ayrıntılı rapor.',
    'export.card.csv.title'           : 'CSV',
    'export.card.csv.desc'            : 'Tüm bulguları tablo biçiminde dışa aktarır. Elektronik tablo uygulamaları için uygundur.',
    'export.card.json.title'          : 'JSON Ham Veri',
    'export.card.json.desc'           : 'Ham doğrulama sonuçlarını JSON biçiminde dışa aktarır. Geliştiriciler için uygundur.',
    'export.card.pdf.title'           : 'PDF Yazdır',
    'export.card.pdf.desc'            : 'Kural toplamlarını ve dosya/satır/alan bazlı kanıtları yazdırmaya uygun PDF olarak oluşturur.',
    'export.card.executive.title'     : 'Yönetici PDF Raporu',
    'export.card.executive.desc'      : 'Yayın hazırlığını, feed profilini, öncelikli bulguları ve iyileştirme planını profesyonel bir raporda sunar.',
    'export.card.executive.language'  : 'Rapor dili',
    'export.card.executive.action'    : 'Raporu Oluştur',
    'export.debug.title'              : 'Teşhis Paketi',
    'export.debug.desc'               : 'Bir hata/anormallik bildirirken paylaşabileceğiniz JSON paketi: uygulama sürümü, tarayıcı, WASM modu, skorlar, kural sayıları ve son işlem/log kayıtları.',
    'export.debug.privacy'            : 'Gizlilik: yalnızca üst-veri ve özet sayılar içerir. Feed içeriği (durak adları, koordinatlar, bulgu mesajları) DAHİL EDİLMEZ ve hiçbir şey sunucuya gönderilmez.',
    'export.note.label'               : 'Not:',
    'export.note.text'                : 'Dışa aktarılan dosyalar yerel olarak oluşturulur; GTFS verisi sunucuya gönderilmez.',
    'export.summary.title'            : 'Özet Tablo',
    'export.summary.datetime'         : 'Tarih-Zaman',
    'export.summary.rule_types'       : 'Tetiklenen kural tipi',
    'export.summary.feed'             : 'Feed',
    'export.summary.copy'             : 'Kopyala',
    'export.summary.copied'           : 'Kopyalandı ✓',
    'export.summary.copy_failed'      : 'Kopyalama başarısız — panoya erişilemedi.',
    'export.summary.desc'             : 'Özeti PNG olarak okuyun; "Kopyala" ile metni (Markdown + kural sayımları) panoya alıp paylaşın. Karşılaştırma için tarih-zaman damgası dahildir.',
    'export.summary.metric'           : 'Bulgu',
    'export.summary.capped'           : 'Cap\'li',
    'export.summary.real'             : 'Gerçek',
    'export.popup_blocked'            : 'Açılır pencere engellendi. Tarayıcı ayarlarını kontrol edin.',
    'export.csv.rule'                 : 'Kural',
    'export.csv.severity'             : 'Önem',
    'export.csv.class'                : 'Sınıf',
    'export.csv.message'              : 'Mesaj',
    'export.csv.entity_id'            : 'Varlık ID',
    'export.csv.file'                 : 'Dosya',
    'export.csv.service'              : 'Çalışma Takvimi',
    'export.csv.row'                  : 'Satır',
    'export.html.doc_title'           : 'GTFS Doğrulama Raporu',
    'export.html.h1'                  : 'GTFS Doğrulama Raporu',
    'export.html.publishability'      : 'Yayınlanabilirlik',
    'export.html.overall_score'       : 'Genel Skor',
    'export.html.pub_score'           : 'Yayın Skoru',
    'export.html.total'               : 'Toplam Bulgu',
    'export.html.h2_findings'         : 'Bulgular',
    'export.html.print_grouped'       : 'Büyük feed’lerde yazdırma için her kural en fazla {examples} ayrıntılı örnekle gösteriliyor ({rules} kural, toplam {total} bulgu). Tam liste HTML, CSV ve JSON çıktılarında yer alır.',
    'export.html.rule_group'          : '{rule} · {severity} · {cls} — {count} bulgu',
    'export.html.rule_examples'       : '{shown} örnek',
    'export.html.publishable_ok'      : 'Yayına Uygun',
    'export.html.publishable_blocked' : 'Yayınlanması Tavsiye Edilmez',
    'export.html.breakdown'           : 'GTFS Geçerliliği {spec} · GTFS Uyumluluğu {interop} · GTFS Kalitesi {quality} · GTFS Analitiği {analytics}',
    'export.html.th.rule'             : 'Kural',
    'export.html.th.severity'         : 'Önem',
    'export.html.th.class'            : 'Sınıf',
    'export.html.th.message'          : 'Mesaj',
    'export.html.th.entity_id'        : 'Varlık ID',
    'export.html.th.entity_type'      : 'Varlık türü',
    'export.html.th.file'             : 'Dosya',
    'export.html.th.service'          : 'Çalışma Takvimi',
    'export.html.th.row'              : 'Satır',
    'export.html.th.field'            : 'Alan',
    'export.html.th.scope'            : 'Kapsam',
    'export.html.th.observed'         : 'Gözlenen değer',
    'export.html.th.expected'         : 'Beklenen değer',
    'export.html.th.details'          : 'Ek teknik ayrıntılar',
    'export.html.th.remediation'     : 'Düzeltme önerisi',
    'export.html.th.blocks'           : 'Etkilediği kapılar',
    'export.html.th.effort'           : 'Temel efor',
    'export.html.th.findings'         : 'bulgu',
    'export.html.no_value'            : 'Belirtilmemiş',
    'export.html.map_link'            : 'Haritada aç',
    'export.html.detail_intro'        : 'Her bulgu; neyin yanlış olduğunu, feed içinde nerede bulunduğunu, mevcut ve beklenen değeri ve önerilen düzeltmeyi gösterir.',
    'export.html.gtfs_jp_profile'     : 'GTFS-JP profili',
    'export.html.no_findings'         : 'Bu feed için bulgu bulunamadı.',
    // Rules page
    'rules.summary'    : '{total} bulgu · {rules} kural',
    'rules.th.severity': 'Önem',
    'rules.th.entity'  : 'Varlık',
    'rules.th.message' : 'Mesaj',
    // Files page
    'files.summary'           : '{count} dosya mevcut',
    'files.general'           : 'Genel (dosyasız bulgular)',
    'files.badge.clean'       : 'Temiz',
    'files.badge.missing'     : 'Eksik',
    'files.badge.required'    : 'Zorunlu',
    'files.badge.unknown'     : 'Spec dışı',
    'files.hint.missing'      : 'Bu dosya feed ZIP\'inde bulunamadı.',
    'files.hint.unknown'      : 'Bu dosya GTFS spesifikasyonunda tanımlı değil.',
    // Fix page file filter
    'fix.filter.file'         : 'Dosya',
    // Entity types
    'entity.Stop'        : 'Durak',
    'entity.Trip'        : 'Sefer',
    'entity.Route'       : 'Hat',
    'entity.Shape'       : 'Güzergah',
    'entity.Agency'      : 'İşletici',
    'entity.Service'     : 'Servis',
    'entity.File'        : 'Dosya',
    'entity.Feed'        : 'Feed',
    'entity.Row'         : 'Satır',
    'entity.Field'       : 'Alan',
    'entity.Transfer'    : 'Aktarma',
    'entity.Pathway'     : 'Geçit',
    'entity.Level'       : 'Kat',
    'entity.Translation' : 'Çeviri',
    'entity.Attribution' : 'Atıf',
    'entity.Fare'        : 'Ücret',
  } as Record<string, string>,

  severity: {
    CRITICAL : 'KRİTİK',
    HIGH     : 'YÜKSEK',
    MEDIUM   : 'ORTA',
    LOW      : 'DÜŞÜK',
    INFO     : 'BİLGİ',
  },

  ruleClass: {
    SPEC      : 'GTFS Geçerliliği',
    INTEROP   : 'GTFS Uyumluluğu',
    QUALITY   : 'GTFS Kalitesi',
    ANALYTICS : 'GTFS Analitiği',
  },

  fatalCode: {
    ZipUnreadable  : 'ZIP dosyası açılamadı',
    Utf8Critical   : 'UTF-8 kodlama hatası',
    NoRequiredFiles: 'Zorunlu dosyalar eksik',
    CsvMalformed   : 'CSV biçim hatası',
    DecompressionLimit: 'Zip bomb koruması devreye girdi',
    ResourceLimit  : 'Notice limiti aşıldı',
    InvalidInput   : 'Geçersiz girdi',
  },

  ruleMessages: {} as Record<string, string>,

  ruleRemediations: {} as Record<string, string>,

  ruleTitles: {
    // ARC
    'ARC_001': 'ZIP arşivi açılamadı',
    'ARC_002': 'Dosya UTF-8 ile okunamıyor',
    'ARC_003': 'İsteğe bağlı dosyada UTF-8 kodlama hatası',
    'ARC_004': 'Zorunlu dosya eksik',
    'ARC_006': 'İsteğe bağlı GTFS dosyası mevcut',
    'ARC_007': 'GTFS dışı tanınmayan dosya',
    'ARC_008': 'Takvim dosyası eksik (calendar.txt ve calendar_dates.txt)',
    'ARC_031': 'translations.txt varken feed_info.txt eksik',
    'ARC_009': 'Opsiyonel dosyada veri satırı yok',
    'ARC_010': 'Dosya UTF-8 BOM içeriyor',
    'ARC_011': 'Dosya boyutu (bilgi)',
    'ARC_012': 'Satır sütun sayısı başlıkla uyuşmuyor',
    'ARC_013': 'CSV ayrıştırma hatası',
    'ARC_014': 'Başlıkta baştaki/sondaki boşluk',
    'ARC_015': 'Yinelenen başlık sütunu',
    'ARC_025': "Zorunlu sütun başlıkta hiç yok (header'da eksik)",
    'ARC_017': 'Bilinmeyen sütun (GTFS spesifikasyonunda tanımlı değil)',
    'ARC_018': 'Boş veri satırı',
    'ARC_019': 'Başlıkta boş sütun adı',
    'ARC_020': 'Önerilen GTFS dosyası eksik (shapes.txt veya feed_info.txt)',
    'ARC_021': 'Alanda yazdırılamaz veya sorunlu karakter',
    'ARC_022': "Dosya satır sayısı 1.000.000 sınırını aşıyor",
    'ARC_023': "ZIP içinde iç içe ZIP dosyası",
    'ARC_024': "GTFS .txt dosyası alt dizinde (parse edilemez)",
    'ARC_026': 'Dosyada hatalı satır sonu karakteri',
    'ARC_027': 'ZIP girdisinde kullanıcı okuma izni yok',
    'ARC_028': "GTFS yayın URL'si .zip dosya adıyla bitmiyor",
    'ARC_029': 'Sıkıştırma koruması: arşiv zip-bomb sınırını aştı',
    'ARC_030': 'Alan değerinde sekme veya satır sonu karakteri',
    'ARC_032': 'Alan değerinde HTML etiketi veya kaçış dizisi',
    'ARC_033': 'Alan değerinde kaçırılmamış tırnak (RFC 4180)',
    'ARC_034': 'Başlık satırı veri satırı olarak tekrarlanmış',
    'ARC_035': 'Zorunlu dosya boş',
    'ARC_036': 'ZIP akış başlığı merkez diziniyle çelişiyor',
    'ARS_002': "areas.txt'te area_id boş",
    'CAL_025': 'Takvim gün alanı boş (değer verilmemiş)',
    'FMD_004': 'fare_media_id boş',
    'NET_004': "networks.txt'te network_id boş",
    'RCT_008': 'rider_category_id boş',
    'XFL_032': "location_groups.txt'te location_group_id boş",
    'XFL_033': "location_group_stops'ta location_group_id boş",
    'XFL_034': "location_group_stops'ta stop_id boş",
    'SAR_003': "stop_areas'ta area_id boş",
    'SAR_004': "stop_areas'ta stop_id boş",
    'TFR_008': "timeframes'te service_id boş",
    'TRP_035': "trips.txt'te service_id boş",
    // BKR
    'BKR_001': 'Önceki gün rezervasyon alanı yasak bağlamda dolu',
    'BKR_002': 'prior_notice_start_day yalnızca prior_notice_last_day ile kullanılabilir',
    'BKR_003': 'prior_notice_start_time yalnızca prior_notice_start_day ile kullanılabilir',
    'BKR_004': 'Anlık rezervasyonda prior_notice alanları yasak',
    'BKR_005': 'prior_notice_duration_max yalnızca booking_type=1 ile geçerli (booking_type=0/2 ile yasak)',
    'BKR_006': 'prior_notice_duration_min geçersiz (≤ 0 veya sayısal değil)',
    'BKR_007': 'booking_type=1 için prior_notice_duration_min zorunlu',
    'BKR_008': 'booking_type=2 için prior_notice_last_day zorunlu',
    'BKR_009': 'booking_type=2 için prior_notice_last_time zorunlu',
    'BKR_010': 'prior_notice_start_day belirtilmişse prior_notice_start_time zorunlu',
    'BKR_011': 'prior_notice_last_day > prior_notice_start_day: rezervasyon penceresi geçersiz',
    'BKR_012': 'booking_type=2 iken prior_notice_duration_min yasak',
    'BKR_013': 'prior_notice_last_time yalnızca prior_notice_last_day ile kullanılabilir',
    'BKR_014': 'prior_notice_service_id yalnızca booking_type=2 ile kullanılabilir',
    'BKR_015': 'prior_notice_service_id bulunamadı (calendar/calendar_dates)',
    'BKR_016': 'booking_type eksik veya geçersiz',
    'BKR_017': 'pickup_booking_rule_id bulunamadı (booking_rules)',
    'BKR_018': 'drop_off_booking_rule_id bulunamadı (booking_rules)',
    'BKR_019': 'booking_rule_id eksik veya yineleniyor',
    'BKR_020': 'booking_url geçersiz',
    'BKR_021': 'info_url geçersiz',
    'BKR_022': 'phone_number geçersiz',
    'BKR_023': 'prior_notice sayı alanı tam sayı değil',
    'BKR_025': 'prior_notice zaman alanı geçerli saat değil',
    'BKR_024': 'booking_type=1 ve duration_max varken prior_notice_start_day yasak',
    // AGN
    'AGN_002': 'agency_name eksik',
    'AGN_003': 'agency_url eksik veya geçersiz',
    'AGN_004': 'agency_timezone eksik veya geçersiz',
    'AGN_005': 'Kuruluşlar arası saat dilimi tutarsızlığı',
    'AGN_006': 'agency_lang geçersiz',
    'AGN_007': 'agency_phone geçersiz',
    'AGN_008': 'agency_fare_url geçersiz',
    'AGN_009': 'agency_email geçersiz',
    'AGN_010': 'agency_id yineleniyor',
    'AGN_011': 'Birden fazla kuruluşta agency_id yok',
    'AGN_012': 'cemv_support geçersiz (agency)',
    'RTS_024': 'cemv_support geçersiz (route)',
    'RTS_025': 'routes.txt agency_id boş (önerilen)',
    'RTS_031': 'route_id eksik',
    'FLG_008': 'fare_product_id eksik',
    'RTS_026': 'Yinelenen kısa hat adı',
    'RTS_027': 'Yinelenen uzun hat adı',
    'RTS_028': 'Flex pencereli rotada continuous_pickup/drop_off yasak',
    'AGN_013': 'Feed dili ve ajans dili uyuşmuyor',
    'AGN_014': 'Birden fazla kuruluş var ama agency.txt agency_id eksik',
    'AGN_015': 'agency_url güvensiz http (https önerilir)',
    'AGN_016': 'agency_phone şüpheli/yer-tutucu numara',
    'AGN_017': "Agency'ler arası agency_lang tutarsız",
    // STP
    'STP_001': 'stop_id yineleniyor',
    'STP_002': 'stop_id boş',
    'STP_003': 'stop_name eksik veya stop_lat/stop_lon aralık dışı (aynı ID altında iki ayrı koşul)',
    'STP_004': 'stop_lat sayısal değil',
    'STP_005': 'stop_lon geçersiz veya aralık dışı',
    'STP_006': 'stop_lat eksik',
    'STP_007': 'stop_lon eksik',
    'STP_008': 'location_type geçersiz',
    'STP_009': 'parent_station bulunamadı',
    'STP_010': 'parent_station location_type=1 değil',
    'STP_011': 'Giriş/çıkış/boarding için parent_station zorunlu',
    'STP_012': "stop_times'ta istasyon veya giriş kullanılmış",
    'STP_013': 'wheelchair_boarding geçersiz',
    'STP_014': 'stop_timezone geçersiz',
    'STP_015': 'level_id bulunamadı',
    'STP_016': 'İki durak tam aynı koordinatta',
    'STP_017': 'İki durak birbirine çok yakın',
    'STP_018': 'Hiç durak yok',
    'STP_019': 'stop_name çok uzun',
    'STP_020': 'Hiç sefer geçmeyen durak',
    'STP_021': 'Boarding area parent\'ı platform değil',
    'STP_022': 'stop_code eksik',
    'STP_023': 'tts_stop_name geçersiz',
    'STP_024': 'stop_access enum aralığı dışında değer (K2 ham alan kontrolü)',
    'STP_025': 'stop_name baştaki veya sondaki boşluk içeriyor',
    'STP_026': 'stop_access geçersiz değer',
    'STP_027': "Pathway istasyonunda stop_access belirtilmemiş",
    'STP_028': 'stop_code çok uzun',
    'STP_029': 'Durak istasyon içinde ama koordinat çok uzakta',
    'STP_030': 'Üst istasyonun alt durağı yok',
    'STP_031': 'Durak adı ve açıklaması aynı',
    'STP_032': 'Pathway bağlantılı platform için parent_station eksik',
    'STP_033': 'Durak zone_id eksik (ücret hesabı için gerekli)',
    'STP_034': "stop_url acente URL'siyle aynı",
    'STP_035': "stop_url hat URL'siyle aynı",
    'STP_036': 'İstasyonun (location_type=1) parent_station değeri var',
    'STP_037': 'Bazı duraklar tekerlekli sandalye erişilebilirliği (wheelchair_boarding) bildirmemiş',
    'STP_038': 'Hiçbir durak tekerlekli sandalye erişilebilirliği (wheelchair_boarding) bildirmemiş',
    'STP_039': 'stop_code birden fazla durakta kullanılıyor',
    'STP_040': 'Durak adı gereksiz genel stop/station sözcüğü içeriyor',
    'STP_044': "platform_code gereksiz 'platform/track' sözcüğü içeriyor",
    'STP_041': 'Alt durak adı üst istasyon adını içermiyor',
    'STP_042': 'stop_url geçersiz',
    'STP_043': 'stop_access yasak bağlamda kullanılmış',
    // RTS
    'RTS_001': 'route_id yineleniyor',
    'RTS_002': 'agency_id bulunamadı',
    'RTS_003': 'route_short_name ve route_long_name ikisi de eksik',
    'RTS_004': 'route_type eksik veya geçersiz',
    'RTS_005': 'route_url geçersiz',
    'RTS_006': 'route_color geçersiz hex renk',
    'RTS_007': 'route_text_color geçersiz hex renk',
    'RTS_008': 'Hat rengi ve metin rengi kontrast düşük',
    'RTS_010': 'route_short_name çok uzun',
    'RTS_011': 'route_long_name çok uzun',
    'RTS_012': 'Seferi olmayan hat',
    'RTS_013': 'continuous_pickup geçersiz',
    'RTS_016': 'Hiçbir aktif servis günü olmayan hat',
    'RTS_017': 'Shape tanımlı olmayan hat',
    'RTS_018': 'continuous_drop_off geçersiz',
    'RTS_019': 'Yinelenen hat adı',
    'RTS_020': "Hat ve acente aynı URL'yi paylaşıyor",
    'RTS_021': 'Kısa hat adı Google Transit eşiğini (6 karakter) aşıyor',
    'RTS_022': 'Uzun hat adı kısa adı içeriyor',
    'RTS_023': 'Hat açıklaması hat adının kopyası',
    // TRP
    'TRP_001': 'trip_id eksik veya yineleniyor',
    'TRP_002': 'route_id bulunamadı',
    'TRP_003': 'service_id bulunamadı',
    'TRP_004': 'shape_id bulunamadı',
    'TRP_005': 'direction_id geçersiz',
    'TRP_006': 'wheelchair_accessible geçersiz',
    'TRP_007': 'bikes_allowed geçersiz',
    'TRP_032': 'cars_allowed geçersiz',
    'TRP_011': 'Sefer yön adı girilmemiş',
    'TRP_012': 'Çift yönlü rotada direction_id eksik',
    'TRP_013': 'Hat tek seferlik',
    'TRP_014': 'trip_short_name çok uzun',
    'TRP_015': 'block_id hiçbir seferi gruplamıyor',
    'TRP_017': "Frekans tabanlı sefer stop_times'ta eksik",
    'TRP_019': 'Continuous servis aktifken shape_id eksik',
    'TRP_020': 'trip_headsign ara durak adıyla eşleşiyor',
    'TRP_021': 'Bisiklet izni (bikes_allowed) belirtilmemiş',
    'TRP_022': 'Block içinde çakışan sefer saatleri',
    'TRP_023': 'Önümüzdeki 7 günde aktif sefer yok',
    'TRP_024': 'Block içinde tutarsız rota tipi',
    'TRP_025': 'Tekerlekli sandalye erişilebilirlik bilgisi eksik seferlerin oranı yüksek',
    'TRP_026': 'Aktif tarihi olmayan sefer (service_id geçerli ama aktif gün seti boş)',
    'TRP_028': 'Bazı seferler tekerlekli sandalye erişilebilirliği işaretlememiş',
    'TRP_029': 'Hiçbir sefer tekerlekli sandalye erişilebilirliği bildirmemiş',
    'CAL_024': 'Takvim önümüzdeki 7 günde aktif değil',
    'TRP_031': 'route_id eksik',
    'TRP_033': "Aynı block_id'yi paylaşan seferler farklı route_type taşıyor",
    // STM
    'STM_001': 'trip_id bulunamadı',
    'STM_002': 'stop_id bulunamadı',
    'STM_003': 'arrival_time geçersiz format',
    'STM_004': 'departure_time geçersiz format',
    'STM_005': 'stop_sequence eksik veya geçersiz',
    'STM_006': 'stop_id eksik (stop_times)',
    'STM_007': 'Kalkış saati varış saatinden önce (departure_time < arrival_time)',
    'STM_008': 'Duraklar arası zaman geriye gidiyor',
    'STM_009': 'pickup_type geçersiz',
    'STM_010': 'drop_off_type geçersiz',
    'STM_012': "Duraklar arası hız gerçekçi değil",
    'STM_013': 'Karışık varış/kalkış zamanları',
    'STM_014': 'Segmentte aşırı hız',
    'STM_015': 'İlk durakta arrival_time eksik',
    'STM_016': 'Son durakta arrival_time eksik',
    'STM_017': 'Sefer saatlerinde güzergah mesafesi eksik',
    'STM_018': 'continuous_pickup geçersiz (stop_times)',
    'STM_019': 'continuous_drop_off geçersiz (stop_times)',
    'STM_020': 'Sıfır geçiş süresi (mesafe > 200m)',
    'STM_021': 'Duraklar arası mesafe sıfır veya negatif',
    'STM_022': 'timepoint geçersiz',
    'STM_024': 'shape_dist_traveled birim tutarsızlığı',
    'STM_025': 'Kısa segment zamanlaması',
    'STM_026': 'Durak arası mesafe aşırı uzun',
    'STM_028': 'Sefer süresi çok uzun',
    'STM_029': 'Sefer süresi çok kısa',
    'STM_030': 'shape_dist_traveled negatif veya sayı değil',
    'STM_032': 'Aynı seferde yinelenen stop_sequence değeri',
    'STM_033': 'Tek duraklı sefer (kullanılamaz)',
    'STM_034': 'Varış veya kalkış zamanından yalnızca biri tanımlı',
    'STM_035': 'Aynı durak ardışık iki kez ziyaret ediliyor (terminal/döngü)',
    'STM_036': 'stop_times trip_id + stop_sequence\'a göre sıralı değil (unsorted_stop_times)',
    'STM_037': 'Flex penceresinde arrival_time/departure_time yasak',
    'STM_038': 'start_pickup_drop_off_window >= end_pickup_drop_off_window',
    'STM_039': 'Flex bağlamında pickup/drop_off penceresi eksik',
    'STM_040': "Flex stop_times'ta pickup/drop_off_booking_rule_id eksik (spec'te Optional)",
    'STM_041': 'stop_id ile location_id/group_id aynı anda kullanılamaz',
    'STM_042': 'stop_headsign Google Transit tarafından desteklenmeyen karakter içeriyor',
    'STM_043': 'Sefer aşırı fazla durağa sahip (>200)',
    'STM_044': 'Feed stop_times satır sayısı 2 milyonu aşıyor',
    'STM_045': 'Sefer kalkış saati servis günü penceresini aşıyor',
    'STM_046': 'trip_id eksik',
    'STM_047': 'timepoint=1 ama arrival_time/departure_time eksik',
    'STM_048': 'GTFS servis gününde gece yarısı sonrası saatler 00:xx yazılmış',
    'STM_049': 'GTFS servis gününde aynı satırda gece yarısı sonrası kalkış 00:xx',
    'STM_050': 'timepoint sütunu var ama değer boş',
    'STM_051': 'Flex penceresinde pickup_type=0/3 yasak',
    'STM_052': 'Flex penceresinde drop_off_type=0 yasak',
    'STM_053': 'Çok sayıda ardışık durakta aynı zaman',
    'STM_054': 'Flex penceresinde continuous_pickup yasak',
    'STM_056': 'shape_dist_traveled artmıyor',
    'STM_058': 'Flex pickup/drop-off penceresi saati geçersiz',
    'STM_059': 'pickup_type/drop_off_type=2 iken booking_rule_id önerilir',
    'STM_060': "Aynı seferde bölge, pencere ve pickup/drop_off eşzamanlı örtüşüyor",
    'STM_061': 'Ardışık olmayan uzak duraklar arası imkânsız hız',
    'STM_055': 'Flex penceresinde continuous_drop_off yasak',
    // PDW
    'PDW_006': "Aynı trip+zone'da örtüşen pickup/drop-off penceresi",
    // LOC
    'LOC_001': "locations.geojson'da bilinmeyen veya geçersiz geometri tipi",
    'LOC_002': "Feature'da geometry null veya eksik",
    'LOC_003': "Feature'da 'id' property eksik",
    'LOC_004': "Polygon ring kapalı değil",
    'LOC_005': "FeatureCollection tamamen boş",
    'LOC_006': "Polygon alanı 500km²'yi aşıyor",
    'LOC_007': "FeatureCollection içinde yinelenen feature 'id'",
    'LOC_010': "Geometry 'coordinates' eksik veya dizi değil — bölge geometrisi çözümlenemez",
    'LOC_011': 'Poligon geçersiz: ring kendini kesiyor veya delik dışarıda',
    'LOC_012': "locations.geojson geçerli bir GeoJSON FeatureCollection değil",
    'LOC_009': "Feature 'properties' nesnesi eksik",
    'LOC_008': "Feature 'type' alanı eksik veya \"Feature\" değil",
    // CAL
    'CAL_001': 'service_id yineleniyor',
    'CAL_002': 'Takvim gün alanı geçersiz değer',
    'CAL_003': 'start_date eksik veya geçersiz format',
    'CAL_004': 'end_date eksik veya geçersiz format',
    'CAL_005': "start_date end_date'den sonra",
    'CAL_006': 'Haftalık bazda tüm günler pasif (calendar_dates ile override mümkün)',
    'CAL_007': 'Servis döneminde boşluk',
    'CAL_008': 'Servis tarihi yakında sona eriyor',
    'CAL_009': "Feed'deki tüm takvim dönemleri sona ermiş",
    'CAL_010': 'Serviste aktif gün sayısı çok az',
    'CAL_011': 'Kullanılmayan servis',
    'CAL_012': 'Yakın gelecekte servis boşluğu var',
    'CAL_013': 'Geçmiş tarihli servis dönemi',
    'CAL_014': 'Servis tarihleri feed_info geçerlilik aralığı dışında',
    'CAL_015': 'Tüm takvim tarihleri gelecekte (bugün aktif sefer yok)',
    'CAL_016': 'Servis çok uzak bir gelecek tarihine kadar uzanıyor',
    'CAL_017': 'Takvim henüz başlamamış (tüm aktif tarihler gelecekte)',
    'CAL_018': 'Servisin aktif haftanın günü yok (tüm günler 0, calendar_dates ile geçersiz kılınan yok)',
    'CAL_019': 'Ham takvim aralığı feed_info geçerlilik penceresini aşıyor',
    'CAL_020': 'Feed geçerlilik penceresi 5 yılı aşıyor',
    'CAL_021': 'Servis bugünü kapsıyor ama yakın günlerde aktif sefer yok',
    'CAL_022': 'service_id eksik',
    'CAL_023': 'Takvim end_date çok ileri (şüpheli)',
    // CLD
    'CLD_001': 'service_id eksik',
    'CLD_002': 'date eksik veya geçersiz format',
    'CLD_003': 'exception_type eksik veya geçersiz',
    'CLD_004': 'calendar_dates-only serviste aktif gün (exception_type=1) tanımlı değil',
    'CLD_005': 'Tarih makul yıl aralığı dışında',
    'CLD_006': 'Çok fazla istisna günü',
    'CLD_007': 'Aşırı takvim istisnası',
    // SHP
    'SHP_001': 'shape_id eksik',
    'SHP_002': 'shape_pt_lat eksik veya geçersiz',
    'SHP_003': 'shape_pt_lon eksik veya geçersiz',
    'SHP_004': 'shape_pt_sequence eksik veya geçersiz',
    'SHP_005': 'shape_dist_traveled geriye gidiyor',
    'SHP_006': 'Güzergah şekli yalnızca tek noktadan oluşuyor',
    'SHP_008': 'shape_pt_sequence yineleniyor',
    'SHP_009': 'Güzergah şekli kendisiyle kesişiyor',
    'SHP_010': 'Tekrarlanan shape noktası (ardışık özdeş koordinat)',
    'SHP_012': 'Güzergah şekli sefer duraklarından çok uzak',
    'SHP_014': 'İlk veya son durak güzergah ucundan uzakta',
    'SHP_015': 'Güzergah şekli istatistiksel olarak çok az nokta',
    'SHP_016': 'Güzergah şekli yön bilgisiyle uyumsuz',
    'SHP_017': 'Durak sırası güzergah şekliyle çelişiyor',
    'SHP_018': 'Güzergah şekli sefer tarafından referanslanmıyor',
    'SHP_019': "Güzergah şeklinin seferleri durak zamanı içermiyor",
    'SHP_020': 'Güzergah şeklinde tekrarlayan nokta',
    'SHP_021': 'shape_dist_traveled negatif veya sayı değil',
    'SHP_022': 'Durak güzergah şeklinde belirsiz konumda',
    'SHP_023': 'shape_dist_traveled aynı değere sahip art arda iki nokta aynı koordinatta',
    'SHP_024': 'Duraktan şekle mesafe shape_dist_traveled ile tutarsız',
    'SHP_025': 'Sefer stop_times mesafesi şeklin toplam mesafesini aşıyor',
    'SHP_026': 'Shape aşırı fazla noktaya sahip (>5000)',
    'SHP_028': 'Aynı shape_dist_traveled, farklı koordinat',
    'SHP_029': 'Aynı shape_dist_traveled, çok küçük koordinat farkı (eşik altı)',
    'SHP_030': "stop_times shape_dist_traveled kullanıyor ancak shapes.txt'te shape mesafeleri eksik",
    // FRQ
    'FRQ_001': 'trip_id bulunamadı',
    'FRQ_002': 'start_time geçersiz',
    'FRQ_003': 'end_time geçersiz',
    'FRQ_004': 'headway_secs eksik veya geçersiz',
    'FRQ_005': "end_time start_time'dan önce",
    'FRQ_006': 'headway_secs çok uzun',
    'FRQ_007': 'exact_times geçersiz',
    'FRQ_008': 'headway_secs sıfır (geçersiz frekans)',
    'FRQ_009': 'Frekans aralığı çok kısa',
    'FRQ_010': 'Çok sık frekans (sıkışma riski)',
    'FRQ_011': 'Frequencies dönemleri zaman aralığı çakışıyor',
    'FRQ_012': 'exact_times=1 iken end_time headway sınırına denk geliyor',
    // TRF
    'TRF_001': 'from_stop_id eksik',
    'TRF_002': 'to_stop_id eksik',
    'TRF_003': 'from_stop_id veya to_stop_id bulunamadı',
    'TRF_004': 'transfer_type geçersiz',
    'TRF_005': 'min_transfer_time eksik',
    'TRF_006': 'from_trip_id bulunamadı',
    'TRF_007': 'to_trip_id bulunamadı',
    'TRF_008': 'from_route_id bulunamadı',
    'TRF_009': 'to_route_id bulunamadı',
    'TRF_010': 'Aktarma süresi çok uzun',
    'TRF_011': 'Aktarma tanımlandı ama mesafe uzak',
    'TRF_012': 'Yinelenen aktarma kaydı',
    'TRF_013': 'Aktarma türü bağlamla uyumsuz',
    'TRF_014': 'in-seat aktarma için sefer yok',
    'TRF_015': 'in-seat aktarma geçersiz',
    'TRF_016': 'Aktarma koşulu çelişkili',
    'TRF_017': 'Sefer aktarması yanlış hat',
    'TRF_018': 'Sefer aktarması aynı seferi gösteriyor',
    'TRF_019': 'In-seat aktarmada farklı route_type',
    'TRF_020': 'Aktarma için gereken yürüme hızı çok yüksek',
    'TRF_021': 'Aktarma uç noktası durak veya istasyon değil',
    'TRF_022': '1-to-n devamlılıkta devam seferlerinin takvimleri çelişiyor',
    'TRF_023': 'n-to-1 devamlılıkta gelen seferlerin takvimleri çelişiyor',
    // GGL
    'GGL_001': 'transfer_type=4/5 Google Transit tarafından desteklenmiyor',
    'GGL_002': 'ic_price (Google-özel) geçersiz değer',
    // FAR
    'FAR_001': 'fare_id yineleniyor',
    'FAR_002': 'price eksik veya geçersiz',
    'FAR_003': 'currency_type eksik',
    'FAR_004': 'payment_method geçersiz',
    'FAR_005': 'transfers geçersiz',
    'FAR_006': 'transfer_duration geçersiz',
    'FAR_008': 'agency_id bulunamadı',
    'FAR_009': 'Ücrete ait hat kuralı yok',
    'FAR_010': 'Çakışan ücret kuralları',
    'FAR_011': 'payment_method eksik',
    'FAR_012': 'fare_id eksik',
    'FAR_013': 'price para biriminin ISO 4217 ondalık basamak sayısını taşımıyor',
    'LVL_008': 'level_id eksik',
    'PTH_020': 'pathway_id eksik',
    'PTH_021': 'from_stop_id eksik',
    'PTH_022': 'to_stop_id eksik',
    'PTH_023': 'pathway_mode eksik',
    'PTH_024': 'is_bidirectional eksik',
    'PTH_025': 'Önerilen pathway length bilgisi eksik',
    'PTH_029': 'Önerilen pathway traversal_time bilgisi eksik',
    'PTH_030': "Boarding area'sı olan platforma pathway atanmış",
    'PTH_026': 'Pathway uç noktası istasyon',
    'PTH_031': "Pathway uç noktası sokaktan erişilen durak (stop_access=1)",
    'PTH_027': 'stair_count geçersiz (sıfır veya tam sayı değil)',
    // FRL
    'FRL_001': 'fare_id bulunamadı',
    'FRL_002': 'route_id bulunamadı',
    'FRL_003': 'origin_id geçersiz',
    'FRL_004': 'destination_id geçersiz',
    'FRL_005': 'contains_id geçersiz',
    'FRL_006': 'Ücret kuralı tanımlı değil',
    'FRL_007': 'Ücret kuralı mantıksal tutarsızlık',
    'FRL_008': 'Tüm hatlar için ücret tanımlı değil',
    // RCT
    'RCT_001': 'rider_category_id yineleniyor',
    'RCT_002': 'rider_category_name eksik',
    'RCT_003': 'is_default_fare_category geçersiz',
    'RCT_004': 'min_age veya max_age geçersiz (GTFS uzantı alanı — resmî spec\'te yok)',
    'RCT_005': "max_age min_age'den küçük",
    'RCT_006': 'fare_product başına varsayılan yolcu kategorisi sayısı bir değil',
    // FMD
    'FMD_001': 'fare_media_id yineleniyor',
    'FMD_002': 'fare_media_type eksik veya geçersiz',
    'FMD_003': 'TransitCard/MobileApp için fare_media_name tavsiye edilir',
    // FPD
    'FPD_001': 'fare_products bileşik anahtarı yineleniyor',
    'FPD_002': 'amount eksik veya sayısal değil',
    'FPD_003': 'currency geçersiz ISO 4217 kodu',
    'FPD_004': 'fare_media_id bulunamadı',
    'FPD_005': 'rider_category_id bulunamadı',
    'FPD_007': 'amount para biriminin ISO 4217 ondalık basamak sayısını taşımıyor',
    // FLG
    'FLG_001': 'fare_product_id bulunamadı',
    'FLG_002': 'network_id bulunamadı',
    'FLG_003': 'from_area_id bulunamadı',
    'FLG_004': 'to_area_id bulunamadı',
    'FLG_005': 'from_timeframe_group_id bulunamadı',
    'FLG_006': 'to_timeframe_group_id bulunamadı',
    'FLG_007': 'rule_priority geçersiz',
    'FLJ_001': 'from_network_id eksik veya bulunamadı',
    'FLJ_002': 'to_network_id eksik veya bulunamadı',
    'FLJ_003': 'from_stop_id eksik veya bulunamadı',
    'FLJ_004': 'to_stop_id eksik veya bulunamadı',
    // FTR
    'FTR_001': 'fare_transfer_type eksik veya geçersiz',
    'FTR_002': 'from_leg_group_id bulunamadı',
    'FTR_003': 'to_leg_group_id bulunamadı',
    'FTR_004': 'fare_product_id bulunamadı',
    'FTR_005': 'duration_limit_type geçersiz',
    'FTR_006': 'duration_limit geçersiz',
    'FTR_007': 'duration_limit_type duration_limit olmadan tanımlı',
    'FTR_008': 'transfer_count geçersiz',
    'FTR_009': 'leg grupları aynıyken transfer_count zorunlu',
    'FTR_010': 'leg grupları farklıyken transfer_count tanımlanamaz',
    'FTR_011': 'duration_limit tanımlı ama duration_limit_type eksik',
    // ARS
    'ARS_001': 'area_id yineleniyor',
    // SAR
    'SAR_001': 'area_id bulunamadı',
    'SAR_002': 'stop_id bulunamadı',
    'XFL_022': 'location_group_id bulunamadı (location_group_stops)',
    'XFL_023': 'stop_id bulunamadı (location_group_stops)',
    'XFL_024': 'location_group_id bulunamadı (stop_times)',
    'XFL_025': 'location_id bulunamadı (locations.geojson)',
    'XFL_026': 'route cemv_support=1 ama uygulanabilir contactless product yok',
    'XFL_027': 'route cemv_support=2 ama uygulanabilir contactless product var',
    'XFL_028': 'agency cemv_support=1 ama contactless media yok',
    'XFL_029': 'route cemv_support=1 ama contactless media yok',
    'XFL_030': 'contactless media var ama cemv_support=1 yok',
    'XFL_031': 'Kimlik çakışması: stop_id / locations.geojson id / location_group_id ortak isim alanını paylaşır',
    // NET
    'NET_001': 'network_id yineleniyor',
    'NET_002': 'route_networks.network_id eksik veya bulunamadı',
    'NET_003': 'route_networks.route_id eksik veya bulunamadı',
    'RCT_007': 'eligibility_url geçersiz',
    'RTS_029': 'route_sort_order geçersiz',
    'RTS_030': 'route_type çekirdek enum dışında genişletilmiş değer',
    'TRN_015': 'record_id ve field_value ikisi de boş',
    'TRN_016': 'field_value hiçbir kayıtla eşleşmiyor — çeviri uygulanmıyor',
    'TRN_017': 'stop_times çevirisinde record_sub_id eksik — hangi satır olduğu belirsiz',
    // TFR
    'TFR_001': 'timeframe_group_id eksik',
    'TFR_002': 'service_id bulunamadı',
    'TFR_003': 'start_time veya end_time format hatası',
    'TFR_004': "end_time start_time'dan küçük",
    'TFR_005': 'Aynı grup ve service_id içinde örtüşen zaman aralıkları',
    'TFR_006': "start_time veya end_time 24:00:00'dan büyük",
    'TFR_007': 'start_time ve end_time yalnızca biri tanımlı',
    // PTH
    'PTH_001': 'pathway_id yineleniyor',
    'PTH_002': 'from_stop_id bulunamadı',
    'PTH_003': 'to_stop_id bulunamadı',
    'PTH_004': 'pathway_mode eksik veya geçersiz',
    'PTH_005': 'is_bidirectional eksik',
    'PTH_006': 'length geçersiz',
    'PTH_007': 'traversal_time geçersiz',
    'PTH_008': 'stair_count eksik',
    'PTH_009': 'max_slope eksik',
    'PTH_010': 'min_width geçersiz',
    'PTH_011': 'Geçit döngü oluşturuyor',
    'PTH_012': 'İstasyona erişilebilir yol yok',
    'PTH_013': 'Erişilebilir yol analizi',
    'PTH_014': 'Geçit istasyon sınırını aşıyor',
    'PTH_015': 'Geçit hedefi erişilemeyen durakta',
    'PTH_016': 'Çıkış kapısı çift yönlü tanımlanmış',
    'PTH_017': 'max_slope sayı değil',
    'PTH_028': 'max_slope yürüme yolu dışında kullanılmış',
    'TRP_034': 'safe_duration alanı sayı değil',
    'PTH_018': 'signposted_as çok uzun',
    'PTH_019': "Generic node tek pathway'e bağlı (dead-end)",
    // LVL
    'LVL_001': 'level_id yineleniyor',
    'LVL_002': 'level_index geçersiz',
    'LVL_003': 'level_name eksik',
    'LVL_004': 'Kullanılmayan level',
    'LVL_005': 'level_name çok uzun',
    'LVL_006': 'Asansör bağlantısındaki durakta level_id eksik',
    'LVL_007': 'level_index eksik',
    // FIN
    'FIN_001': 'feed_publisher_name eksik',
    'FIN_002': 'feed_publisher_url eksik veya geçersiz',
    'FIN_003': 'feed_lang eksik',
    'FIN_004': 'default_lang geçersiz',
    'FIN_005': 'feed_start_date geçersiz',
    'FIN_006': 'feed_end_date geçersiz format (geçmiş tarih için FIN_010 kullanın)',
    'FIN_007': 'feed_version eksik',
    'FIN_008': 'feed_contact_email geçersiz',
    'FIN_009': 'feed_contact_url geçersiz',
    'FIN_010': 'Feed geçerlilik süresi dolmuş',
    'FIN_012': "feed_start_date feed_end_date'den sonra",
    'FIN_013': 'fare_attributes.agency_id önerilen ama eksik',
    'FIN_014': 'Feed geçerlilik tarihleri (feed_start_date/feed_end_date) eksik',
    'FIN_015': 'Birden fazla feed_info kaydı',
    'FIN_016': 'feed_start_date gelecekte (feed henüz aktif değil)',
    'FIN_017': 'Feed çok uzak gelecekte sona eriyor',
    'FIN_018': 'feed_contact_email ve feed_contact_url ikisi de eksik',
    'FIN_019': "Feed'in geçerlilik süresi 7 gün içinde dolacak",
    'FIN_020': 'Feed geçerlilik penceresi 7 günden kısa',
    // TRN
    'TRN_001': 'table_name geçersiz değer',
    'TRN_002': 'field_name bu tablo için geçersiz',
    'TRN_003': 'language geçersiz',
    'TRN_004': 'record_id bulunamadı',
    'TRN_005': 'Çeviri yineleniyor',
    'TRN_006': 'Çeviri kaydı çelişkili',
    'TRN_007': 'Çeviri feed_lang ile aynı dilde',
    'TRN_008': 'translation değeri boş',
    'TRN_009': 'record_id ve field_value aynı anda kullanılamaz',
    'TRN_010': 'record_sub_id geçersiz',
    'TRN_011': 'field_name çevrilebilir değil',
    'TRN_013': "feed_info çevirisinde kimlik alanı kullanılamaz",
    'TRN_014': 'record_sub_id yalnızca stop_times için geçerli',
    // ATR
    'ATR_001': 'attribution_id eksik',
    'ATR_002': 'organization_name eksik',
    'ATR_003': 'Attribution rolü tanımlanmamış',
    'ATR_004': 'is_producer geçersiz',
    'ATR_005': 'is_operator geçersiz',
    'ATR_006': 'is_authority geçersiz',
    'ATR_007': 'attribution_url geçersiz',
    'ATR_008': 'attribution_email geçersiz',
    'ATR_009': 'attribution hedef alanları birlikte kullanılmış',
    'ATR_010': 'agency_id bulunamadı',
    'ATR_011': 'route_id bulunamadı',
    'ATR_012': 'trip_id bulunamadı',
    // XFL
    'XFL_002': 'Seferin stop_times kaydı yok',
    'XFL_006': 'service_id yalnızca iptal istisnası içeriyor (aktif gün yok)',
    'XFL_011': 'Takvim tarihleri feed_info aralığı dışında',
    'XFL_012': 'Çalıştırılabilir seferi olmayan hat',
    'XFL_013': 'shape_id birden fazla yönde kullanılıyor',
    'XFL_014': 'Geçersiz çeviri referansı (kaynak kayıt bulunamadı)',
    'XFL_015': "Attribution'da geçersiz referans",
    'XFL_016': "Çeviri feed_info'ya referans veriyor ama feed_info.txt eksik",
    'XFL_017': 'route_cemv_support ile agency_cemv_support çelişiyor',
    'XFL_019': 'Ağ tanımı iki ayrı dosyada (routes.network_id + route_networks.txt)',
    'XFL_020': "Transfers'de geçersiz (from_trip_id/to_trip_id, route_id) çifti",
    'XFL_021': "Transfers'de geçersiz (from_trip_id/to_trip_id, stop_id) çifti",
    // OPR
    'OPR_001': 'Hat sefer sıklığı düşük',
    'OPR_003': 'Sefer sıkışması (minimum aralık çok küçük)',
    'OPR_004': 'Hafta sonu sefer yok',
    'OPR_005': 'Sıradışı sefer sıklığı',
    'OPR_006': 'Seferde çok az durak (işlevsel değil)',
    'OPR_007': 'Sefer içinde tekrarlayan durak deseni',
    'OPR_008': 'Birden fazla segmentte aşırı hız',
    'OPR_009': 'Gece seferi başlangıç saati çok geç',
    'OPR_010': 'Hatta erişilebilirlik veya bisiklet politikası çelişiyor',
    'OPR_011': 'Serviste aktif gün yok',
    'OPR_012': 'Servis boşluğu',
    'OPR_013': 'Hat tek yönde işliyor (karşı yön tanımlı değil)',
    'OPR_014': 'Ortalama aktarma süresi uzun',
    'OPR_015': 'Hat yalnızca tek güzergahla işliyor',
    'OPR_016': 'Feed genelinde aktif servis yok',
    'OPR_017': 'Sefer çok kısa mesafe',
    'OPR_019': 'Rota takvim çakışması (aynı günde birden fazla servis)',
    'OPR_020': 'Rota exception günü çakışması',
    'OPR_021': 'Takvim override çakışması: override ve base eş zamanlı aktif',
    'OPR_022': 'Takvim override uygulanmamış: override gününde base servis çalışıyor',
    'OPR_023': 'Takvim override boşluğu: pencere içinde hiçbir servis aktif değil',
    'OPR_024': 'Hat çok fazla sefer içeriyor',
    'OPR_025': 'Ortalama sefer süresi 60 saniyeden kısa',
    // GEO
    'GEO_002': 'Durak feed medianından çok uzakta',
    'GEO_006': 'Güzergah şeklinde büyük atlama',
    'GEO_007': 'Güzergah şeklinde kritik atlama (3× eşik)',
    'GEO_009': 'Durak shape güzergahından çok uzakta',
    'GEO_012': 'Duraksallar kümelenmesi (çok yakın duraklar)',
    'GEO_013': 'Feed coğrafi kapsam özeti',
    'GEO_014': 'Feed coğrafi kapsamı çok geniş',
    'GEO_015': 'Durak koordinatları Japonya sınırları dışında (feed_lang: ja)',
    'GEO_016': 'Durak Null Island yakınında (|lat|<0.1 VE |lon|<0.1)',
    'GEO_017': 'Shape noktası Null Island yakınında',
    'GEO_018': 'Tüm feed durakları 200m içinde (test verisi)',
    'GEO_019': 'Durak tam sayı koordinatlara sahip (hassasiyet sıfır)',
    'GEO_020': 'Shape dejenere — tüm noktalar aynı koordinatta',
    'GEO_021': 'Durakların %30+ koordinatı paylaşıyor (sistematik hata)',
    'GEO_022': 'Durak enlemi kutba aşırı yakın (|lat| > 89)',
    // DQ
    'DQ_003':  'Hat açıklaması eksik',
    'DQ_004':  "Hat URL'si eksik",
    'DQ_005':  'Geçerli servis dönemi yok',
    'DQ_005b': 'Hiçbir seferin durak zamanı yok',
    'DQ_005c': 'Koordinatsız durak oranı çok yüksek',
    'DQ_006':  'Şekil olmayan sefer oranı çok yüksek',
    'DQ_009':  'Seferlerde durak zamanı yok',
    'DQ_010':  'Acente hiçbir hatta kullanılmıyor',
    'DQ_011':  'Yalnızca bir durak var',
    'DQ_012':  'Çok fazla acente, agency_id kullanılmıyor',
    'DQ_013':  'Çok az sefer',
    'DQ_016':  'Değerde fazladan boşluk karakteri',
    'DQ_017':  'Şüpheli koordinat değeri',
    'DQ_018':  'Önerilen alanda tamamen büyük harf (all-caps)',
    'DQ_019':  'Önerilen alanda tamamen küçük harf (all-lowercase)',
    'DQ_020':  'Önerilen alan eksik veya boş',
    'DQ_021':  'Birincil anahtar yineleniyor — genel ikincil sinyal (STP_001/RTS_001 ile örtüşebilir)',
    'DQ_022':  "Durakların %80+'i aynı stop_name değerini paylaşıyor",
    // VAT
    'VAT_001': 'Hat güzergah benzerliği (muhtemel kopya hat)',
    'VAT_002': 'Aktarma merkezi tanımsız — çok sayıda hat geçiyor ama aktarma yok',
    'VAT_003': 'Sefer süresi istatistiksel aykırı değer',
    'VAT_005': 'İzole durak kümesi — ağ grafiğinde ana bileşenden kopuk duraklar',
    'VAT_006': 'Hizmet yoğunluğu dengesizliği — tek hat feed sefer sayısının büyük bölümünü oluşturuyor',
    'VAT_007': 'Terminus aktarma eksikliği — terminal durağa başka hat geliyor ama aktarma tanımlı değil',
    'VAT_008': "Aynı shape feed hatlarının %30+'ında kullanılıyor",
    'JPN_001': 'GTFS-JP: durak adında kana (ja-Hrkt) okuması eksik',
    'JPN_002': "GTFS-JP: jp_office_id office_jp.txt'te tanımlı değil",
    'JPN_003': "GTFS-JP: agency_jp.agency_id agency.txt'te tanımlı değil",
    'JPN_004': 'GTFS-JP: translations.txt eksik',
    'JPN_005': 'GTFS-JP: office_jp.office_name boş',
    'JPN_006': 'GTFS-JP: fare_attributes eksik veya farklı ücret profilleri için fare_rules eksik',
    'JPN_007': 'GTFS-JP: feed_info.txt eksik',
    'JPN_008': 'GTFS-JP: hat adının kana okuması eksik',
    'JPN_009': 'GTFS-JP: trip_headsign kana okuması eksik',
    'JPN_010': 'GTFS-JP: işletici adının kana okuması eksik',
    'JPN_011': 'GTFS-JP: tek işletici olsa bile agency_id zorunlu',
    'JPN_012': 'GTFS-JP: agency_jp.agency_id eksik',
    'JPN_013': 'GTFS-JP: agency_zip_number biçimi geçersiz',
    'JPN_014': 'GTFS-JP: office_jp.office_id eksik veya tekrarlı',
    'JPN_015': 'GTFS-JP legacy: routes_jp.route_id eksik, tekrarlı veya routes.txt içinde tanımsız',
    'JPN_016': 'GTFS-JP: pattern_jp veya legacy routes_jp route_update_date geçersiz',
    'JPN_017': 'GTFS-JP: jp_pattern_id eksik veya tekrarlı',
    'JPN_018': 'GTFS-JP: jp_pattern_id tanımsız',
    'JPN_019': 'GTFS-JP: kana çeviri referansı geçersiz',
    'JPN_020': 'GTFS-JP: ofis iletişim biçimi şüpheli',
    'JPN_021': 'GTFS-JP: kana çevirisi tutarsız',
    'JPN_022': 'GTFS-JP v4: ana alan zorunluluğu eksik',
    'JPN_023': 'GTFS-JP: feed_lang ja değil',
    'JPN_024': 'GTFS-JP: agency_lang ja değil',
    'JPN_025': 'GTFS-JP: agency_timezone Asia/Tokyo değil',
    'JPN_026': 'GTFS-JP: ücret para birimi JPY değil',
    'JPN_028': 'GTFS-JP v3: zorunlu kana çevirisi eksik',
    'JPN_029': 'GTFS-JP v4: önerilen kana çevirisi eksik',
    'JPN_030': 'GTFS-JP v3: zorunlu Japonca çeviri eksik',
    'JPN_028.v3.aggregate': 'GTFS-JP v3: {table_name}.{field} = {source_value} için zorunlu ja-Hrkt çevirisi eksik; {affected_records} satırı etkiliyor. Örnekler: {example_record_ids}.',
    'JPN_028.v3.aggregate_both': 'GTFS-JP v3: {table_name}.{field} = {source_value} için zorunlu ja-Hrkt ve language=ja çevirileri eksik; {affected_records} satırı etkiliyor. Örnekler: {example_record_ids}.',
    'JPN_030.v3.aggregate': 'GTFS-JP v3: {table_name}.{field} = {source_value} için zorunlu language=ja çevirisi eksik; {affected_records} satırı etkiliyor. Örnekler: {example_record_ids}.',
    'JPN_027': 'GTFS-JP V3: route_type 3 değil',
    'JPN_031': 'GTFS-JP: bölgeye bağlı ücret kullanılan durakta zone_id eksik',
  } as Record<string, string>,
};

export default tr;
export type LocaleShape = typeof tr;
