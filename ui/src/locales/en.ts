...
const en: LocaleShape = {
  ui: {
    ...
    'upload.error_size'       : 'File too large ({mb} MB). Maximum allowed size: {max} MB.',
    ...
    'cfg.feed_info_expiry_warning_days.label' : 'Feed Info Expiry Warning',
    'cfg.feed_info_expiry_warning_days.unit'  : 'days',
    'cfg.feed_info_expiry_warning_days.desc'  : 'FIN_019 warning horizon for feed_end_date; separate from the CAL_008 threshold',
    ...
    'domain.gtfs_jp.coverage'    : 'GTFS-JP {profile} · 100% Machine-Checkable Coverage (human review and recommendation-only provisions excluded)',
    'domain.gtfs_jp.coverage_tip': 'Covers all strong and soft MLIT provisions enforced by the profile and backed by an Analyzer rule; human review, external verification, and recommendation-only provisions whose absence is not a conformance violation are outside the denominator. The feed’s official GTFS-JP version is not inferred automatically.',
    ...
    'export.card.html.desc'           : 'Detailed report with the file, row, field, observed/expected values, and remediation for every finding.',
    ...
    'export.card.pdf.desc'            : 'Print-ready PDF with rule totals and file/row/field-level evidence for remediation.',
    ...
    'export.html.print_grouped'       : 'For large feeds, printing shows at most {examples} detailed examples per rule ({rules} rules, {total} findings in total). The full list is in the HTML, CSV, and JSON exports.',
    ...
    'export.html.th.entity_type'      : 'Entity type',
    'export.html.th.file'             : 'File',
    'export.html.th.service'          : 'Service Calendar',
    'export.html.th.row'              : 'Row',
    'export.html.th.field'            : 'Field',
    'export.html.th.scope'            : 'Scope',
    'export.html.th.observed'         : 'Observed value',
    'export.html.th.expected'         : 'Expected value',
    'export.html.th.details'          : 'Additional technical details',
    'export.html.th.remediation'     : 'Remediation',
    'export.html.th.blocks'           : 'Affected gates',
    'export.html.th.effort'           : 'Base effort',
    'export.html.th.findings'         : 'findings',
    'export.html.no_value'            : 'Not provided',
    'export.html.map_link'            : 'Open in map',
    'export.html.detail_intro'        : 'Each finding shows what is wrong, where it occurs in the feed, the observed and expected values, and the recommended fix.',
    'export.html.gtfs_jp_profile'     : 'GTFS-JP profile',
    'export.html.no_findings'         : 'No findings were detected for this feed.',
    // Rules page
  ...
  ruleMessages: {
    ...
    'ARC_031': 'translations.txt is present but feed_info.txt is missing; the specification requires it in that case.',
    ...
    'ARC_032': "Field values in '{entity_id}' contain HTML markup (for example '{observed_value}'); affected columns: {field}.",
    'ARC_033': "'{entity_id}' does not follow RFC 4180: {observed_value}. A value containing a quotation mark or a comma must be quoted, and every quotation mark inside it doubled.",
    'ARC_034': "'{entity_id}' line {line}: the header row is repeated as data.",
    'ARC_035': "Required GTFS file '{entity_id}' contains no data rows.",
    'ARC_036': '{observed_value} ZIP entries have a stream header that contradicts the central directory; tools reading the archive as a stream cannot open it.',
    ...
    'ARC_020': "Recommended file '{file}' is missing (shapes.txt or feed_info.txt).",
    'ARC_021': "Non-printable or problematic character {observed_value} in '{entity_id}'.",
    ...
    'RTS_031': "routes.txt line {line}: route_id is missing — the row has no identity and no trip can reference it.",
    'FLG_008': "fare_leg_rules.txt line {line}: fare_product_id is missing — the rule does not say which product it applies.",
    ...
    'STP_005': '{observed_value} stops have an invalid or out-of-range stop_lon in this feed.',
    ...
    'STP_022': "stop_code is missing for {affected_stops} stop(s), e.g. {example_stops}.",
    ...
    'STP_032': '{observed_value} pathway-connected platforms have no parent_station in this feed.',
    ...
    'TRP_005': '{observed_value} trips have an invalid direction_id in this feed.',
    ...
    'TRP_015': 'None of the {observed_value} block_id values groups two trips: all {trips_with_block} trips carrying a block_id sit alone in their own block, so the field duplicates trip_id.',
    ...
    'STM_007': "departure_time is before arrival_time on {affected_rows} stop_times row(s), e.g. trip {example_trips}.",
    'STM_008': "Trip '{entity_id}' has {observed_value} backward-time segments; the first is shown in the details.",
    ...
    'STM_032': "Duplicate stop_times row on {affected_rows} row(s), e.g. trip {example_trips}.",
    ...
    'STM_048': "{observed_value} trips wrote after-midnight times as 00:xx; GTFS requires 24:00:00+ on the service day (the values are normalized to 24:xx for analysis).",
    'STM_049': "{observed_value} stops wrote an after-midnight departure as 00:xx; GTFS requires 24:00:00+ on the service day.",
    ...
    'STM_056': "shape_dist_traveled does not increase on {affected_rows} stop_times row(s), e.g. trip {example_trips}.",
    ...
    'LOC_006': "Feature {observed_value} in locations.geojson covers a very large area (~{observed_value} km²).",
    ...
    'CLD_003': "{observed_value} rows in '{entity_id}' have a missing or invalid exception_type.",
    ...
    'SHP_010': "{affected_shapes} shape(s) have consecutive identical coordinates, e.g. {example_shapes}.",
    ...
    'FRQ_007': '{observed_value} frequency trips have invalid exact_times.',
    ...
    'TRF_009': "Transfer: to_route_id '{observed_value}' not found in routes.txt.",
    ...
    'TRF_021': "Transfer: {field} '{observed_value}' has a location_type other than stop (0) or station (1).",
    'TRF_022': "Trip '{entity_id}' is linked to several continuing trips whose calendars conflict: {observed_value}.",
    'TRF_023': "Trip '{entity_id}' is the continuation of several trips whose calendars conflict: {observed_value}.",
    ...
    'NET_002': "Route network row for '{entity_id}': network_id '{observed_value}' is missing or not defined in networks.txt.",
    ...
    'PTH_007': "Pathway '{entity_id}' has {observed_value} rows with invalid traversal_time.",
    ...
    'PTH_012': '{observed_value} platforms cannot be reached from an entrance in this feed.',
    ...
    'PTH_026': "Pathway '{entity_id}': {field} is a station (location_type=1); endpoints must be platforms, entrances/exits, generic nodes or boarding areas.",
    'PTH_031': "Pathway '{entity_id}': {field} '{observed_value}' is marked stop_access=1 (reached directly from the street), so it cannot be a pathway endpoint.",
    'PTH_027': "Pathway '{entity_id}': stair_count '{observed_value}' is not a non-zero integer.",
    'PTH_017': "Pathway '{entity_id}': max_slope '{observed_value}' is not a number.",
    'PTH_028': "Pathway '{entity_id}': max_slope is used outside a walkway or moving sidewalk.",
    'TRP_034': "Trip '{entity_id}': {field} '{observed_value}' is not a decimal number.",
    ...
    'TRN_005': "Duplicate translation for '{observed_value}'.",
    'TRN_006': "Translation conflict for '{observed_value}'.",
    ...
    // ATR
    'ATR_001': '{observed_value} in attributions.txt have no attribution_id; those records cannot be told apart when repeated.',
    'ATR_002': "Attribution '{entity_id}': organization_name is missing.",
    'ATR_003': '{observed_value} in attributions.txt define no attribution role (is_producer, is_operator, is_authority).',
    ...
    'ATR_007': "Attribution: attribution_url '{observed_value}' is invalid.",
    'ATR_008': "Attribution '{entity_id}': attribution_email is invalid.",
    'ATR_009': '{observed_value} in attributions.txt set more than one of agency_id, route_id and trip_id.',
    ...
    'XFL_006': "Services with only cancellation exceptions and no calendar entry: {observed_value}. They can never be active.",
    ...
    'XFL_015': "Attribution: reference '{observed_value}' is invalid.",
    ...
    'JPN_006': 'The GTFS-JP fare coverage is missing or incomplete.',
    'JPN_006.v3': 'GTFS-JP v3 requires fare_attributes.txt; distinct fare profiles also require fare_rules.txt.',
    'JPN_006.v4': 'GTFS-JP v4 normally requires fare_attributes.txt, except when the fare cannot be represented by this format; an empty or invalid file does not establish that exception.',
    'JPN_006.auto': 'The detected GTFS-JP fare coverage is missing or incomplete.',
    'JPN_007': "feed_info.txt is missing — GTFS-JP requires it (feed_lang=ja and publisher information).",
    'JPN_008': "Route '{entity_id}': {field} is missing a kana (ja-Hrkt) reading expected by GTFS-JP.",
    'JPN_008.v3': "Route '{entity_id}': {field} is missing a kana (ja-Hrkt) reading required by GTFS-JP v3.",
    'JPN_008.v4': "Route '{entity_id}': {field} is missing a kana (ja-Hrkt) reading recommended by GTFS-JP v4.",
    'JPN_008.auto': "Route '{entity_id}': {field} is missing the kana (ja-Hrkt) reading expected by the detected GTFS-JP scope.",
    'JPN_009': "Trip '{entity_id}': trip_headsign is missing a kana (ja-Hrkt) reading expected by GTFS-JP.",
    'JPN_009.v3': "Trip '{entity_id}': trip_headsign is missing a kana (ja-Hrkt) reading required by GTFS-JP v3.",
    'JPN_009.v4': "Trip '{entity_id}': trip_headsign is missing a kana (ja-Hrkt) reading recommended by GTFS-JP v4.",
    'JPN_009.auto': "Trip '{entity_id}': trip_headsign is missing the kana (ja-Hrkt) reading expected by the detected GTFS-JP scope.",
    'JPN_010': "Agency '{entity_id}': agency_name is missing a kana (ja-Hrkt) reading expected by GTFS-JP.",
    'JPN_010.v3': "Agency '{entity_id}': agency_name is missing a kana (ja-Hrkt) reading required by GTFS-JP v3.",
    'JPN_010.v4': "Agency '{entity_id}': agency_name is missing a kana (ja-Hrkt) reading recommended by GTFS-JP v4.",
    'JPN_010.auto': "Agency '{entity_id}': agency_name is missing the kana (ja-Hrkt) reading expected by the detected GTFS-JP scope.",
    'JPN_011': 'GTFS-JP requires agency_id even for a single agency; {observed_value} agency record(s) are missing it.',
    'JPN_012': 'agency_jp.txt row is missing agency_id.',
    'JPN_013': "Agency '{entity_id}': agency_zip_number must be exactly 7 ASCII digits when present.",
    'JPN_014': "office_jp.txt office_id '{entity_id}' is missing or duplicated.",
    'JPN_015': "routes_jp.txt route_id '{entity_id}' is missing, duplicated, or not defined in routes.txt.",
    'JPN_016': "GTFS-JP: route_update_date in pattern_jp.txt or legacy routes_jp.txt is not a valid YYYYMMDD date.",
    'JPN_017': "pattern_jp.txt jp_pattern_id '{entity_id}' is missing or duplicated.",
    'JPN_018': "Trip '{entity_id}': jp_pattern_id is not defined in pattern_jp.txt.",
    ...
    'JPN_023': "The selected GTFS-JP profile requires feed_lang='ja'; observed '{observed_value}'.",
    'JPN_024': "The selected GTFS-JP profile requires agency_lang='ja'; observed '{observed_value}'.",
    'JPN_025': "The selected GTFS-JP profile requires agency_timezone='Asia/Tokyo'; observed '{observed_value}'.",
    'JPN_026': "The selected GTFS-JP profile requires currency_type='JPY'; observed '{observed_value}'.",
    ...
    'JPN_032': "GTFS-JP V3 agency_id '{observed_value}' is not a 13-digit Corporate Number format.",
    ...
    'JPN_028.v3.aggregate': 'GTFS-JP v3: required ja-Hrkt translation missing for {table_name}.{field} = {source_value}; affects {affected_records} rows. Examples: {example_record_ids}.',
    'JPN_028.v3.aggregate_both': 'GTFS-JP v3: required ja-Hrkt and language=ja translations missing for {table_name}.{field} = {source_value}; affects {affected_records} rows. Examples: {example_record_ids}.',
    'JPN_030.v3.aggregate': 'GTFS-JP v3: required language=ja translation missing for {table_name}.{field} = {source_value}; affects {affected_records} rows. Examples: {example_record_ids}.',
    ...
    'ARC_028': "The GTFS publishing URL does not end with a .zip filename: '{observed_value}'.",
    'ARS_002': 'area_id is required in areas.txt.',
    'BKR_023': "{field} '{observed_value}' cannot be read as an integer.",
    'BKR_024': 'prior_notice_start_day is forbidden when booking_type=1 and prior_notice_duration_max is set.',
    'CAL_025': "The '{field}' field is empty for service_id '{entity_id}'; day fields are required (0 or 1).",
    'FAR_013': '{observed_value} in fare_attributes.txt do not carry the ISO 4217 decimal places of their currency (expected {expected_value}).',
    'FMD_004': 'fare_media_id is required.',
    'FPD_007': '{observed_value} in fare_products.txt do not carry the ISO 4217 decimal places of their currency (expected {expected_value}).',
    'FRQ_012': "With exact_times=1, end_time ('{observed_value}') sits a full headway away from the last departure, leaving the final trip ambiguous.",
    'LOC_011': "A polygon in '{file}' is invalid under OpenGIS Simple Features 6.1.11.",
    'LOC_012': "'{file}' is not a valid GeoJSON FeatureCollection.",
    'NET_004': 'network_id is required in networks.txt.',
    'PTH_025': '{observed_value} walkway, fare gate, or exit gate records are missing the recommended length.',
    'PTH_029': '{observed_value} moving walkway, escalator, or elevator records are missing the recommended traversal_time.',
    'PTH_030': "The pathway connects to platform {field}='{observed_value}', but that platform defines boarding areas — pathways must connect to the boarding areas.",
    ...
    'STP_041': "The name of child stop '{entity_id}' does not include its parent station name.",
    'STP_044': "platform_code '{observed_value}' contains a redundant platform/track word; it should hold the identifier only.",
    'TFR_008': 'service_id is required in timeframes.txt.',
    'TRF_020': 'The transfer requires walking at {observed_value}, which is faster than a plausible walking speed.',
    'TRN_016': "The 'field_value' of {observed_value} translation records matches no row in the target file, so those translations are never applied. For example: {examples}.",
    'TRN_017': 'A stop_times translation sets record_id but omits record_sub_id, so the row it belongs to is ambiguous.',
    'TRP_035': 'service_id is required; a trip left empty is attached to no calendar.',
    'XFL_032': 'location_group_id is required in location_groups.txt.',
    'XFL_033': 'location_group_id is required in location_group_stops.txt.',
    'XFL_034': 'stop_id is required in location_group_stops.txt.',
  } as Record<string, string>,
  ...
  ruleRemediations: {
    ...
    'TRP_015': 'Share one block_id across trips run consecutively by the same vehicle. A single-trip block is valid per the specification, so this finding is not about that.',
    ...
    'VAT_002': 'Review whether an explicit transfer in transfers.txt is needed for this stop.',
    'VAT_003': 'Review the stop_times.txt values and trip routes for this statistical outlier.',
    ...
    'VAT_007': 'Review whether an explicit transfer in transfers.txt is needed at this terminus.',
    ...
    'JPN_006': 'Add fare_attributes.txt; when fare profiles differ by route or area, add fare_rules.txt to map them.',
    ...
    'JPN_012': 'Set agency_jp.txt agency_id to an agency_id defined in agency.txt.',
    'JPN_013': 'Enter agency_zip_number as 7 ASCII digits without a hyphen.',
    'JPN_014': 'Give every office_jp.txt row a non-empty, unique office_id.',
    'JPN_015': 'Use a unique route_id defined in routes.txt for every routes_jp.txt row.',
    'JPN_016': 'Set route_update_date in pattern_jp.txt, or in a legacy routes_jp.txt feed, to a valid YYYYMMDD date.',
    'JPN_017': 'Give every pattern_jp.txt row a unique jp_pattern_id.',
    'JPN_018': 'Point trips.jp_pattern_id to a jp_pattern_id defined in pattern_jp.txt.',
    ...
    'JPN_021': 'Keep one consistent kana translation for each target and include Hiragana or Katakana.',
    'JPN_022': 'Fill in the required GTFS-JP v4 core field.',
    'JPN_023': 'Set feed_info.feed_lang to ja.',
    'JPN_024': 'Set each non-empty agency_lang value to ja.',
    'JPN_025': 'Set agency_timezone to Asia/Tokyo.',
    'JPN_026': 'Set fare_attributes.currency_type to JPY.',
    ...
    'JPN_028.v3.aggregate': 'Add the matching language=ja-Hrkt translation for this source value.',
    'JPN_028.v3.aggregate_both': 'Add both the matching language=ja-Hrkt and language=ja translations for this source value.',
    'JPN_030.v3.aggregate': 'Add the matching language=ja translation for this source value.',
    'JPN_027': 'Use route_type=3 for the V3 bus profile, or select the profile appropriate for multimodal data.',
    'JPN_026.v3.aggregate': 'Set fare_attributes.currency_type to JPY for these records.',
    ...
    'JPN_032': 'Use 13 ASCII digits for agency_id; append an underscore and non-empty branch id only when needed.',
    ...
    'AGN_006': 'Use a valid BCP-47 language code (e.g., tr, en-US), with no leading or trailing spaces.',
    ...
    'AGN_011': 'Fill in agency_id for all routes and fare records when multiple agencies are present.',
    ...
    'RTS_031': 'Give every row in routes.txt a unique route_id.',
    'FLG_008': 'Give every row in fare_leg_rules.txt a fare_product_id defined in fare_products.txt.',
    // RCT
    ...
    'STM_038': 'Set start_pickup_drop_off_window to be strictly earlier than end_pickup_drop_off_window.',
    ...
    'STM_047': 'Provide the missing arrival_time and/or departure_time on exact time points (timepoint=1).',
    'STM_047.missing_arrival': 'Provide arrival_time on this exact time point (timepoint=1).',
    'STM_047.missing_departure': 'Provide departure_time on this exact time point (timepoint=1).',
    'STM_047.missing_both': 'Provide both arrival_time and departure_time on this exact time point (timepoint=1).',
    ...
    'STM_061': 'Check the arrival and departure times between the distant stop pair; usually a time block has shifted or a past-midnight time was written without the 24+ hour form.',
    ...
    'GGL_002': 'Set ic_price to -1 (unknown) or a zero-or-positive number.',
    // PTH (field-level)
    ...
    'PTH_031': 'Set the stop stop_access to 0, or remove this pathway record.',
    ...
    'FIN_008': 'Use a valid e-mail address for feed_contact_email, with no leading or trailing spaces.',
    ...
    'NET_002': 'Use a network_id that is defined in networks.txt.',
    ...
    'ARC_031': 'Add feed_info.txt and fill in feed_lang — it says which language the translations are relative to.',
    ...
    'ARC_032': 'Remove HTML tags and character escapes from field values; GTFS fields carry plain text and consumers display this markup literally.',
    'ARC_033': 'Quote any field value that contains a quotation mark or a comma, and double every quotation mark inside it (\"12\"\" Street\").',
    'ARC_034': 'Remove the repeated header row in the middle of the file; it usually comes from two files being concatenated.',
    'ARC_035': 'Add the real records to the file; leaving it empty is equivalent to omitting it.',
    'ARC_036': 'Repack the archive with a standard ZIP tool; the data-descriptor flag and the sizes in the local header must be consistent.',
    ...
    // CAL
    'CAL_009': 'Update the calendar end_date values so at least one service has future active dates.',
    ...
    'TRF_021': 'Reference a stop or a station in transfers.txt instead of an entrance, generic node or boarding area.',
    'TRF_022': "In a 1-to-n continuation every to_trip_id must share one service_id. If these are separate continuations, their calendars must not overlap on any day.",
    'TRF_023': "In an n-to-1 continuation every from_trip_id must share one service_id. If these are separate continuations, their calendars must not overlap on any day.",
    ...
    // v0.1.4: rules that had no dictionary entry (SDK English output)
    'ARC_026': 'Rewrite the line endings as LF or CRLF.',
    'ARC_027': "Add the user-read (0400) bit to the ZIP entry's Unix permissions.",
    'ARC_028': 'Publish the feed at a permanent URL that ends with an explicit .zip filename.',
    'ARS_002': 'Give every area a unique area_id.',
    'BKR_024': 'Clear prior_notice_start_day, or remove prior_notice_duration_max.',
    'CAL_025': 'Enter 0 or 1 for every day of the week.',
    'FAR_013': 'Write amounts with the decimal places of their currency (for example 0.90 for EUR, 150 for JPY).',
    'FMD_004': 'Give every fare media a unique fare_media_id.',
    'FPD_007': 'Write amounts with the decimal places of their currency (for example 0.90 for EUR, 150 for JPY).',
    'FRQ_012': 'Set end_time after the last departure but BEFORE the next one.',
    'LOC_011': 'Fix the polygon rings so they do not self-intersect, and keep holes inside the outer ring.',
    'LOC_012': 'Regenerate locations.geojson as a valid GeoJSON FeatureCollection document.',
    'NET_004': 'Give every network a unique network_id.',
    'PTH_025': 'Add length in metres to records whose pathway_mode is 1, 6, or 7.',
    'PTH_029': 'Add traversal_time in seconds to records whose pathway_mode is 3, 4, or 5.',
    'PTH_030': "Connect the pathway to the platform's boarding area (location_type=4).",
    'RCT_008': 'Give every rider category a unique rider_category_id.',
    'SAR_003': 'Attach the row to an area_id.',
    'SAR_004': 'Attach the row to a stop_id.',
    'STM_053': 'Check the consecutive stop_times values; if the wait is real, write the times so they show it.',
    'STM_059': 'Point this field at the matching rule in booking_rules.txt for on-demand pickup or drop-off.',
    'STP_039': 'Make every rider-facing stop_code unique within the feed.',
    'STP_040': 'Remove the generic word; if it is part of the official name, turn this opt-in profile off.',
    'STP_041': 'Use the parent station name inside the child stop name, or turn the profile off if local naming differs.',
    'STP_044': "Leave only the platform identifier in platform_code (for example '5' instead of 'Gleis 5').",
    'TFR_008': 'Attach every timeframe to a service_id defined in the calendar.',
    'TRF_020': 'Raise min_transfer_time, or check the transfer stops.',
    'TRN_016': 'Match field_value exactly to the real content of the target field, including leading/trailing spaces and letter case.',
    'TRN_017': 'Enter the matching stop_sequence value in record_sub_id.',
    'TRP_035': 'Assign every trip a service_id defined in calendar.txt or calendar_dates.txt.',
    'XFL_032': 'Give every location group a unique location_group_id.',
    'XFL_033': 'Attach the row to a location_group_id.',
    'XFL_034': 'Attach the row to a stop_id.',
  } as Record<string, string>,
  ...
  ruleTitles: {
    ...
    'ARC_031': 'feed_info.txt missing while translations.txt is present',
    'ARC_009': 'Optional file has no data rows',
    ...
    'ARC_032': 'HTML markup or character escape in a field value',
    'ARC_033': 'Unescaped quotation mark in a field value (RFC 4180)',
    'ARC_034': 'Header row repeated as a data row',
    'ARC_035': 'Required file is empty',
    'ARC_036': 'ZIP stream header contradicts the central directory',
    'ARS_002': 'areas.txt has an empty area_id',
    'CAL_025': 'Calendar day field is empty',
    'FMD_004': 'fare_media_id is empty',
    'NET_004': 'networks.txt has an empty network_id',
    'RCT_008': 'rider_category_id is empty',
    'XFL_032': 'location_groups.txt has an empty location_group_id',
    'XFL_033': 'location_group_stops has an empty location_group_id',
    'XFL_034': 'location_group_stops has an empty stop_id',
    'SAR_003': 'stop_areas has an empty area_id',
    'SAR_004': 'stop_areas has an empty stop_id',
    'TFR_008': 'timeframes has an empty service_id',
    ...
    'RTS_031': 'route_id missing',
    'FLG_008': 'fare_product_id missing',
    'AGN_013': 'Feed language and agency language mismatch',
    ...
    'STP_044': 'platform_code contains a redundant platform/track word',
    ...
    'TRP_015': 'block_id groups no trips',
    ...
    'STM_048': 'After-midnight service-day times written as 00:xx (GTFS Spec)',
    'STM_049': 'After-midnight departure written as 00:xx on the same row (GTFS Spec)',
    ...
    'STM_061': 'Impossible speed between distant non-consecutive stops',
    ...
    'LOC_012': 'locations.geojson is not a valid GeoJSON FeatureCollection',
    ...
    'TRF_021': 'Transfer endpoint is not a stop or station',
    'TRF_022': 'Conflicting calendars in a 1-to-n trip continuation',
    'TRF_023': 'Conflicting calendars in an n-to-1 trip continuation',
    ...
    'NET_002': 'route_networks.network_id is missing or not found',
    ...
    'PTH_026': 'Pathway endpoint is a station',
    'PTH_031': 'Pathway endpoint is a street-accessed stop (stop_access=1)',
    'PTH_027': 'stair_count is invalid (zero or not an integer)',
    'PTH_017': 'max_slope is not a number',
    'PTH_028': 'max_slope used outside a walkway',
    'TRP_034': 'safe_duration field is not a number',
    ...
    'JPN_012': 'GTFS-JP: agency_jp agency_id missing',
    'JPN_013': 'GTFS-JP: agency_zip_number format invalid',
    'JPN_014': 'GTFS-JP: office_jp office_id missing or duplicated',
    'JPN_015': 'GTFS-JP legacy: routes_jp route_id invalid',
    'JPN_016': 'GTFS-JP: pattern_jp or legacy routes_jp route_update_date invalid',
    'JPN_017': 'GTFS-JP: jp_pattern_id missing or duplicated',
    'JPN_018': 'GTFS-JP: jp_pattern_id undefined',
    ...
    'JPN_023': 'GTFS-JP: feed_lang is not ja',
    'JPN_024': 'GTFS-JP: agency_lang is not ja',
    'JPN_025': 'GTFS-JP: agency_timezone is not Asia/Tokyo',
    'JPN_026': 'GTFS-JP: fare currency is not JPY',
    ...
    'JPN_032': 'GTFS-JP V3: agency_id is not in Corporate Number format',
