(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const d of document.querySelectorAll('link[rel="modulepreload"]'))o(d);new MutationObserver(d=>{for(const h of d)if(h.type==="childList")for(const p of h.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&o(p)}).observe(document,{childList:!0,subtree:!0});function s(d){const h={};return d.integrity&&(h.integrity=d.integrity),d.referrerPolicy&&(h.referrerPolicy=d.referrerPolicy),d.crossOrigin==="use-credentials"?h.credentials="include":d.crossOrigin==="anonymous"?h.credentials="omit":h.credentials="same-origin",h}function o(d){if(d.ep)return;d.ep=!0;const h=s(d);fetch(d.href,h)}})();const H={page:"upload",result:null,configDelta:sessionStorage.getItem("gtfs-config-delta")??"",fileName:""};function Q(){return H}function _e(e,t){H.result=e,H.fileName=t,H.page="domain"}function C(e){H.page=e}function de(e){H.configDelta=e,sessionStorage.setItem("gtfs-config-delta",e)}const q={CRITICAL:"KRİTİK",HIGH:"YÜKSEK",MEDIUM:"ORTA",LOW:"DÜŞÜK",INFO:"BİLGİ"},U={SPEC:"GTFS Geçerliliği",INTEROP:"GTFS Uyumluluğu",QUALITY:"GTFS Kalitesi",ANALYTICS:"GTFS Analitiği"},ye={ZipUnreadable:"ZIP dosyası açılamadı",Utf8Critical:"UTF-8 kodlama hatası",NoRequiredFiles:"Zorunlu dosyalar eksik",CsvMalformed:"CSV biçim hatası",ResourceLimit:"Notice limiti aşıldı",InvalidInput:"Geçersiz girdi"},j={CRITICAL:"var(--color-critical)",HIGH:"var(--color-high)",MEDIUM:"var(--color-medium)",LOW:"var(--color-low)",INFO:"var(--color-info)"},F=new Map;let ke=1,W=pe();function pe(){const e=new Worker(new URL(""+new URL("validator-worker-Bzfurpmn.js",import.meta.url).href,import.meta.url),{type:"module"});return e.onmessage=ve,e.onerror=Se,e}function ve(e){const t=e.data,s=F.get(t.id);if(s){if(t.type==="file-list"){s.callbacks?.onFileList?.(t.files);return}if(t.type==="file-done"){s.callbacks?.onFileDone?.(t.name,t.rows,t.bytes);return}if(t.type==="stage"){s.callbacks?.onStageDone?.(t.stage,t.elapsed_ms);return}t.type==="result"&&(F.delete(t.id),t.ok?s.resolve(t.result):s.reject(t.error))}}function Se(e){const t={code:"ResourceLimit",message:e.message||"Arka plan doğrulama işçisi beklenmedik şekilde durdu."};for(const[,s]of F)s.reject(t);F.clear(),W.terminate(),W=pe()}const $e=5*60*1e3;function Te(e,t){return setTimeout(()=>{F.has(e)&&(F.delete(e),t({code:"ResourceLimit",message:"Doğrulama 5 dakika içinde tamamlanamadı. Daha küçük bir feed deneyin."}))},$e)}function we(e,t,s){const o=ke++;return new Promise((d,h)=>{const p=Te(o,h);F.set(o,{resolve:u=>{clearTimeout(p),d(u)},reject:u=>{clearTimeout(p),h(u)},callbacks:s}),W.postMessage({id:o,type:"validate",buffer:e,configDelta:t},[e])})}const ue={K1:"K1 — Dosya Ayrıştırma",K2:"K2 — Kural Doğrulama",K3:"K3 — Varlık Haritası",K4:"K4 — Çapraz Kontrol",K5:"K5 — Türetilmiş Veri",K6:"K6 — İstatistik Analizi",K7:"K7 — Rapor Oluşturma"},K=["K1","K2","K3","K4","K5","K6","K7"],Le=[{key:"max_speed_bus_kmh",label:"Maks. Otobüs Hızı",unit:"km/s",type:"float",def:120,min:60,max:200,desc:"Otobüs seferleri için maksimum izin verilen hız"},{key:"max_speed_tram_kmh",label:"Maks. Tramvay Hızı",unit:"km/s",type:"float",def:100,min:40,max:160,desc:"Tramvay seferleri için maksimum izin verilen hız"},{key:"max_speed_metro_kmh",label:"Maks. Metro Hızı",unit:"km/s",type:"float",def:150,min:80,max:250,desc:"Metro seferleri için maksimum izin verilen hız"},{key:"max_speed_rail_kmh",label:"Maks. Demiryolu Hızı",unit:"km/s",type:"float",def:300,min:100,max:400,desc:"Demiryolu seferleri için maksimum izin verilen hız"},{key:"max_speed_ferry_kmh",label:"Maks. Feribot Hızı",unit:"km/s",type:"float",def:80,min:20,max:150,desc:"Feribot seferleri için maksimum izin verilen hız"},{key:"max_speed_cablecar_kmh",label:"Maks. Teleferik Hızı",unit:"km/s",type:"float",def:30,min:10,max:60,desc:"Teleferik/füniküler için maksimum izin verilen hız"},{key:"min_transfer_time_sec",label:"Min. Aktarma Süresi",unit:"sn",type:"int",def:180,min:30,max:1800,desc:"Transferler için minimum bağlantı süresi"},{key:"max_transfer_distance_m",label:"Maks. Aktarma Mesafesi",unit:"m",type:"float",def:500,min:50,max:2e3,desc:"Transfer geçerli sayılmak için maksimum mesafe"},{key:"max_shape_jump_km",label:"Maks. Güzergah Sıçraması",unit:"km",type:"float",def:10,min:1,max:50,desc:"Art arda güzergah noktaları arasındaki maksimum mesafe"},{key:"stop_too_close_m",label:"Çok Yakın Durak Eşiği",unit:"m",type:"float",def:5,min:1,max:20,desc:"Bu mesafeden yakın duraklar tekrar sayılır"},{key:"stop_far_from_shape_m",label:"Durağın Güzergah'a Uzaklığı",unit:"m",type:"float",def:100,min:20,max:500,desc:"Durağın güzergahından en fazla bu kadar uzakta olabilir"},{key:"stop_far_from_parent_m",label:"Üst İstasyona Uzaklık Eşiği",unit:"m",type:"float",def:100,min:10,max:1e3,desc:"Durak, üst istasyonundan en fazla bu kadar uzakta olabilir"},{key:"feed_expiry_warning_days",label:"Son Kullanma Uyarısı",unit:"gün",type:"int",def:30,min:1,max:60,desc:"Feed bu kadar günden az kalmışsa uyarı üretilir"},{key:"service_gap_days",label:"Servis Boşluğu Eşiği",unit:"gün",type:"int",def:7,min:3,max:30,desc:"Bu günden uzun servis kesintisi işaretlenir"},{key:"max_trip_duration_hours",label:"Maks. Sefer Süresi",unit:"saat",type:"float",def:24,min:8,max:72,desc:"Tek bir seferin maksimum süresi"},{key:"min_trip_duration_sec",label:"Min. Sefer Süresi",unit:"sn",type:"int",def:60,min:10,max:300,desc:"Tek bir seferin minimum süresi"},{key:"max_headway_warning_min",label:"Maks. Sefer Aralığı",unit:"dk",type:"int",def:240,min:60,max:720,desc:"Bu dakikadan uzun aralık uyarı üretir"},{key:"bunching_threshold_min",label:"Sıkışma Eşiği",unit:"dk",type:"int",def:2,min:1,max:10,desc:"Bu dakikadan kısa aralık sıkışma sayılır"}],fe={"stops.txt":"durak","routes.txt":"hat","trips.txt":"sefer","stop_times.txt":"geçiş zamanı","shapes.txt":"güzergah","agency.txt":"işletici","calendar.txt":"takvim","calendar_dates.txt":"özel gün","frequencies.txt":"frekans","transfers.txt":"aktarma","pathways.txt":"geçit","levels.txt":"kat","attributions.txt":"atıf","translations.txt":"çeviri"};function ze(e){const t=Q(),s=t.result!==null,o=s?"Farklı GTFS Yükle":"GTFS Yükle";e.innerHTML=`
    <div class="up-grid">
      ...
      <div class="up-cell up-drop">
        <div id="drop-zone" class="drop-zone" role="button" tabindex="0" aria-label="ZIP dosyası yükle">
          ...
          <label id="upload-btn-label" class="btn btn-primary btn-sm" for="file-input">${R(o)}</label>
        ...
        <div id="stage-rows" class="lp-rows">
          ${K.map(d=>`
            <div class="lp-stage-row ${s?"done":"waiting"}" data-stage="${d}">
              ...
              <span class="lp-label">${R(ue[d]??d)}</span>
    ...
    <div class="sc-chips">${h}</div>`}function Me(e){return Le.map(t=>{const s=e[t.key]!==void 0?e[t.key]:t.def,o=e[t.key]!==void 0;return`
      <div class="sf-row${o?" sf-overridden":""}">
        <label class="sf-label" title="${R(t.desc)}">${R(t.label)}</label>
        ...
        <div class="sf-desc">${R(t.desc)} (varsayılan: ${t.def})</div>
      </div>`}).join("")}function Re(e){if(!e||e==="{}")return{};try{const t=JSON.parse(e);if(typeof t!="object"||t===null||Array.isArray(t))return{};const s={};for(const[o,d]of Object.entries(t))typeof d=="number"&&isFinite(d)&&(s[o]=d);return s}catch{return{}}}function Pe(e){const t=e.querySelector("#drop-zone"),s=e.querySelector("#file-input"),o=e.querySelector("#upload-error");s.addEventListener("change",()=>{const u=s.files?.[0];u&&le(u,e,o)}),t.addEventListener("dragover",u=>{u.preventDefault(),t.classList.contains("loading")||t.classList.add("drag-over")}),t.addEventListener("dragleave",()=>t.classList.remove("drag-over")),t.addEventListener("drop",u=>{if(u.preventDefault(),t.classList.remove("drag-over"),t.classList.contains("loading"))return;const a=u.dataTransfer?.files[0];a&&le(a,e,o)}),t.addEventListener("keydown",u=>{(u.key==="Enter"||u.key===" ")&&!t.classList.contains("loading")&&s.click()}),e.querySelector("#btn-report")?.addEventListener("click",()=>{C("domain"),O()});const d=e.querySelector("#settings-toggle"),h=e.querySelector("#settings-body"),p=e.querySelector("#settings-arrow");d.addEventListener("click",()=>{const u=!h.classList.contains("hidden");h.classList.toggle("hidden",u),p.textContent=u?"▶":"▼"}),e.querySelector("#btn-apply-settings")?.addEventListener("click",()=>Ae(e)),e.querySelector("#btn-reset-settings")?.addEventListener("click",()=>Ee(e)),e.querySelectorAll(".sf-reset").forEach(u=>{u.addEventListener("click",()=>{const a=u.dataset.key,l=e.querySelector(`.sf-input[data-key="${CSS.escape(a)}"]`);l&&(l.value=l.dataset.def,l.closest(".sf-row")?.classList.remove("sf-overridden"),u.style.visibility="hidden")})}),e.querySelectorAll(".sf-input").forEach(u=>{u.addEventListener("change",()=>{const a=e.querySelector(`.sf-reset[data-key="${CSS.escape(u.dataset.key)}"]`);a&&(a.style.visibility="visible"),u.closest(".sf-row")?.classList.add("sf-overridden")})})}function Ae(e){const t={};e.querySelectorAll(".sf-input").forEach(d=>{const h=d.dataset.key,p=parseFloat(d.dataset.def),u=d.dataset.type==="int"?parseInt(d.value,10):parseFloat(d.value);!isNaN(u)&&u!==p&&(t[h]=u)});const s=Object.keys(t).length>0?JSON.stringify(t):"";de(s);const o=e.querySelector("#settings-status");o.textContent="Ayarlar kaydedildi. Yeni ZIP yükleyince uygulanır.",o.className="settings-status ok",setTimeout(()=>{o.className="settings-status hidden"},3e3)}function Ee(e){e.querySelectorAll(".sf-input").forEach(s=>{s.value=s.dataset.def,s.closest(".sf-row")?.classList.remove("sf-overridden");const o=e.querySelector(`.sf-reset[data-key="${CSS.escape(s.dataset.key)}"]`);o&&(o.style.visibility="hidden")}),de("");const t=e.querySelector("#settings-status");t.textContent="Varsayılanlara döndürüldü.",t.className="settings-status ok",setTimeout(()=>{t.className="settings-status hidden"},3e3)}const Fe=512*1024*1024;async function le(e,t,s){if(!e.name.endsWith(".zip")){Y(s,{code:"InvalidInput",message:"Yalnızca .zip uzantılı dosyalar kabul edilir."});return}if(e.size>Fe){Y(s,{code:"InvalidInput",message:`Dosya çok büyük (${(e.size/1048576).toFixed(1)} MB). Maksimum izin verilen boyut: 512 MB.`});return}s.className="upload-status hidden",Ge(t,e.name,e.size);try{const o=await e.arrayBuffer(),d=await we(o,Q().configDelta,{onFileList:h=>{const p=t.querySelector("#file-rows");p&&(p.innerHTML="",h.forEach(u=>{const a=document.createElement("div");a.className="lp-file-row reading",a.dataset.file=u.name;const l=Math.round(u.uncompressed_size/1024);a.innerHTML=`
            <span class="lp-icon">○</span>
            <span class="lp-label">${R(u.name)}</span>
            <span class="lp-status">${Z(l)} okunuyor…</span>`,p.appendChild(a)}))},onFileDone:(h,p,u)=>{const l=t.querySelector("#file-rows")?.querySelector(`[data-file="${CSS.escape(h)}"]`);if(!l)return;l.className="lp-file-row done",l.querySelector(".lp-icon").textContent="✓";const r=fe[h]??"satır";l.querySelector(".lp-status").textContent=`${p.toLocaleString("tr-TR")} ${r} · ${Z(Math.round(u/1024))}`},onStageDone:(h,p)=>{const u=t.querySelector(`[data-stage="${CSS.escape(h)}"]`);if(!u)return;u.className="lp-stage-row done",u.querySelector(".lp-icon").textContent="✓",u.querySelector(".lp-status").textContent=p<1e3?`${p} ms`:`${(p/1e3).toFixed(1)} sn`;const a=K.indexOf(h);if(a>=0&&a<K.length-1){const l=t.querySelector(`[data-stage="${CSS.escape(K[a+1])}"]`);l?.classList.contains("waiting")&&(l.className="lp-stage-row running",l.querySelector(".lp-icon").textContent="⋯",l.querySelector(".lp-status").textContent="çalışıyor…")}}});_e(d,e.name),He(t,d)}catch(o){ge(t),Y(s,o)}}function Ge(e,t,s){const o=e.querySelector("#drop-zone"),d=e.querySelector("#file-input"),h=e.querySelector("#upload-btn-label"),p=e.querySelector("#score-panel");p&&(p.classList.add("hidden"),p.innerHTML="");const u=e.querySelector("#btn-report");u&&(u.disabled=!0),o.classList.add("loading"),d.disabled=!0,h.setAttribute("aria-disabled","true"),h.style.opacity="0.5",h.style.pointerEvents="none";const a=(s/1048576).toFixed(1),l=o.querySelector(".drop-primary");l&&(l.textContent=`${t} (${a} MB) işleniyor…`);const r=e.querySelector("#stage-rows");r&&(r.innerHTML=K.map((n,i)=>`
      <div class="lp-stage-row ${i===0?"running":"waiting"}" data-stage="${n}">
        <span class="lp-icon">${i===0?"⋯":"○"}</span>
        <span class="lp-label">${R(ue[n]??n)}</span>
        <span class="lp-status">${i===0?"çalışıyor…":"bekliyor"}</span>
      </div>`).join(""))}function ge(e){const t=e.querySelector("#drop-zone"),s=e.querySelector("#file-input"),o=e.querySelector("#upload-btn-label");t.classList.remove("loading"),s.disabled=!1,o.removeAttribute("aria-disabled"),o.style.opacity="",o.style.pointerEvents=""}function He(e,t){ge(e);const s=e.querySelector("#upload-btn-label");s.textContent="Farklı GTFS Yükle";const o=e.querySelector("#score-panel");o.className="score-compact",o.innerHTML=be(t);const d=e.querySelector("#btn-report");d.disabled=!1,d.addEventListener("click",()=>{C("domain"),O()},{once:!0});const h=e.querySelector("#file-rows");h&&t.metrics.file_stats.length>0&&(h.innerHTML=he(t.metrics.file_stats))}function Y(e,t){const s=ye[t.code]??t.code;e.className="upload-status error",e.innerHTML=`<strong>${R(s)}</strong><p>${R(t.message)}</p>`}function Z(e){return e>=1024?`${(e/1024).toFixed(1)} MB`:`${e} KB`}function R(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Oe(e,t){const{r1:s,r5:o}=t.reports,d=t.metrics,h=Be(t);e.innerHTML=`
    <div class="report-page">
      ${qe(o,h)}
      ${De(s,t)}
      ${Ce(o)}
      ${xe(d)}
    </div>`}function qe(e,t){const s=ae(e.pub_score),o=ae(e.score);return`
    <div class="rpt-score-row">
      ...
      ${Ke(t)}
    </div>`}function ae(e){return e>=80?"#16a34a":e>=60?"#d97706":"#dc2626"}function De(e,t){const s=new Map(t.notices.map(o=>[o.id,o]));if(!e.publishable){const o=e.blocker_notice_ids.slice(0,5).map(d=>{const h=s.get(d);return h?`<li><code>${D(h.rule_id)}</code> — ${D(h.title||h.message.slice(0,80))}</li>`:""}).join("");return`
      ...
      </div>`}if(e.conditional){const o=e.conditional_blocker_notice_ids.slice(0,5).map(d=>{const h=s.get(d);return h?`<li><code>${D(h.rule_id)}</code> — ${D(h.title||h.message.slice(0,80))}</li>`:""}).join("");return`
    ...
    </div>`}function Be(e){const t={CRITICAL:0,HIGH:0,MEDIUM:0,LOW:0,INFO:0};for(const s of e.notices)s.severity in t&&t[s.severity]++;return t}function xe(e){const s=[{label:"Durak",value:e.stop_count.toLocaleString("tr-TR")},{label:"Hat",value:e.route_count.toLocaleString("tr-TR")},{label:"Sefer",value:e.trip_count.toLocaleString("tr-TR")},{label:"Güzergah",value:e.shape_count.toLocaleString("tr-TR")},{label:"Aktif Servis Günü",value:e.active_service_days.toLocaleString("tr-TR")},{label:"Ort. Günlük Sefer",value:e.avg_daily_trips.toFixed(0)}].map(d=>`
    ...
    <tr>
      <td><code>${D(d.name)}</code></td>
      <td class="num">${d.rows.toLocaleString("tr-TR")}</td>
      <td class="num">${Ie(d.bytes)}</td>
    </tr>`).join("");return`
    <div class="card">
    ...
    </div>`,document.body.appendChild(T),T.querySelector(".mm-close").addEventListener("click",V),T.addEventListener("click",e=>{e.target===T&&V()}),document.addEventListener("keydown",e=>{e.key==="Escape"&&V()}))}const Ye={blocker:"Engelleyici",conditional_blocker:"Koşullu Engelleyici",interop:"Uyumluluk",propagation:"Zincirleme","quick-win":"Kolay Düzeltme",quality:"Kalite Öncelikli",widespread:"Yaygın",analytics:"Analitik",hard:"Zor",single:"Tek Örnek","high-impact":"Yüksek Etki"};function Ve(e,t){const s=new Map(t.notices.map(a=>[a.id,a])),o=t.reports.r9.items.reduce((a,l)=>a+l.score_delta,0),d=o>0?(100-t.reports.r5.score)/o:1,h=t.reports.r9.items.reduce((a,l)=>a+l.pub_score_delta,0),p=h>0?(100-t.reports.r5.pub_score)/h:1,u=new Map(t.reports.r9.items.map(a=>[a.rule_id,{qualDelta:a.score_delta*d,pubDelta:a.pub_score_delta*p,count:a.affected_instance_count}]));e.innerHTML=`
    <section class="page-fix">
      ${We(t.reports.r9.items,s,d,p,t.capped_totals)}
      ${Ze(t,s,u,t.name_index)}
    </section>`}function We(e,t,s,o,d){if(e.length===0)return'<div class="card"><h2>Düzeltme Kuyruğu (R9)</h2><p class="empty">Düzeltilecek bulgu yok.</p></div>';const h={};for(const u of t.values())h[u.rule_id]=(h[u.rule_id]??0)+1;const p=e.map((u,a)=>{const l=u.notice_ids[0]?t.get(u.notice_ids[0]):void 0,r=l?.severity??"INFO",n=u.labels.map(S=>`<span class="label-badge label-${S}">${Ye[S]??S}</span>`).join(""),i=u.pub_score_delta*o,c=u.score_delta*s,f=i>=.05?`<span class="pub-delta">+${i.toFixed(1)}</span>`:'<span class="muted-text">—</span>',b=c>=.05?`<span class="score-delta">+${c.toFixed(1)}</span>`:'<span class="muted-text">—</span>',g=d[u.rule_id],m=g!=null?`<span class="cap-total">${g.toLocaleString("tr-TR")}</span>`:'<span class="muted-text">—</span>',_=h[u.rule_id]??u.affected_instance_count,k=`
    ...
    </div>`}function Ze(e,t,s,o){const d=e.reports.r2.items;if(d.length===0)return'<div class="card"><h2>Tüm Bulgular (R2)</h2><p class="empty">Bulgu yok.</p></div>';const u=`
    ...
    </div>`,ot(e)}function lt(e){const t=new Map;for(const s of e){t.has(s.rule_id)||t.set(s.rule_id,{rule_id:s.rule_id,max_severity:s.severity,rule_class:s.rule_class,notices:[]});const o=t.get(s.rule_id);o.notices.push(s),(G[s.severity]??9)<(G[o.max_severity]??9)&&(o.max_severity=s.severity)}return Array.from(t.values()).sort((s,o)=>{const d=(G[s.max_severity]??9)-(G[o.max_severity]??9);return d!==0?d:o.notices.length-s.notices.length})}function at(e){const t=j[e.max_severity],s=q[e.max_severity],o=U[e.rule_class]??e.rule_class,d=e.rule_class.toLowerCase(),p=[...e.notices].sort((a,l)=>(G[a.severity]??9)-(G[l.severity]??9)).map(a=>{const l=rt(a.message),r=l.length>160?l.slice(0,160)+"…":l,n=j[a.severity]??"#666",i=tt[a.entity_type]??a.entity_type,c=a.entity_id?`<span class="muted-text">${A(i)}</span> <code class="entity-id">${A(a.entity_id)}</code>`:`<span class="muted-text">${A(i)}</span>`;return`
      <tr>
        <td style="color:${n};white-space:nowrap">${q[a.severity]}</td>
        <td>${c}</td>
    ...
    </div>`}function ot(e){e.querySelectorAll(".rule-acc-header").forEach(t=>{t.addEventListener("click",()=>{const o=t.closest(".rule-acc-section").querySelector(".rule-acc-body"),d=t.querySelector(".acc-arrow"),h=!o.hidden;o.hidden=h,d.textContent=h?"▶":"▼",t.setAttribute("aria-expanded",String(!h))})})}function rt(e){return e.replace(/\btrip_id\b/g,"sefer").replace(/\bstop_sequence\b/g,"Durak Sırası").replace(/\barrival_time\b/g,"Varış zamanı").replace(/\bdeparture_time\b/g,"Kalkış zamanı").replace(/\bheadway\b/g,"Sefer Aralığı")}function A(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function nt(e,t,s){e.innerHTML=`
    ...
    </section>`,e.querySelector("#btn-export-html").addEventListener("click",()=>dt(t,s)),e.querySelector("#btn-export-csv").addEventListener("click",()=>it(t,s)),e.querySelector("#btn-export-json").addEventListener("click",()=>ct(t,s)),e.querySelector("#btn-export-pdf").addEventListener("click",()=>pt(t,s))}function re(e){const t=e==null?"":String(e),s=/^[=+\-@]/.test(t)?`'${t}`:t;return s.includes(",")||s.includes('"')||s.includes(`
`)?`"${s.replace(/"/g,'""')}"`:s}function it(e,t){const s=["Kural","Önem","Sınıf","Mesaj","Varlık ID","Dosya","Satır"],o=e.notices.map(u=>[u.rule_id,q[u.severity]??u.severity,U[u.rule_class]??u.rule_class,u.message,u.entity_id??"",u.file??"",u.line!=null?String(u.line):""].map(re).join(",")),h="\uFEFF"+[s.map(re).join(","),...o].join(`\r
`),p=new Blob([h],{type:"text/csv; charset=utf-8"});ee(p,t.replace(/\.zip$/i,"-report.csv"))}function ct(e,t){const s=new Blob([JSON.stringify(e,null,2)],{type:"application/json"});ee(s,t.replace(/\.zip$/i,"-report.json"))}function me(e,t){const{r1:s,r5:o}=e.reports,d=s.publishable?s.conditional?"Koşullu Yayına Uygun":"Yayına Uygun":"Yayınlanması Tavsiye Edilmez",h=e.notices.map(p=>`
    <tr>
      <td>${E(p.rule_id)}</td>
      <td>${q[p.severity]}</td>
...
<body>
  ...
  <table>
...
</html>`}function dt(e,t){const s=me(e,t),o=new Blob([s],{type:"text/html; charset=utf-8"});ee(o,t.replace(/\.zip$/i,"-report.html"))}function pt(e,t){const s=me(e,t),o=window.open("","_blank");if(!o){alert("Açılır pencere engellendi. Tarayıcı ayarlarını kontrol edin.");return}o.document.write(s),o.document.close(),o.focus(),o.print()}function ee(e,t){const s=URL.createObjectURL(e),o=document.createElement("a");o.href=s,o.download=t,o.click(),URL.revokeObjectURL(s)}function E(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const ne={domain:"Rapor",fix:"Ayrıntı ve Düzeltme",rules:"Kategori Bazlı",export:"Dışa Aktar"};function ut(){localStorage.getItem("gtfs-theme")==="dark"&&document.documentElement.classList.add("dark")}function ie(){const e=document.documentElement.classList.toggle("dark");localStorage.setItem("gtfs-theme",e?"dark":"light"),ft()}function ft(){const e=document.documentElement.classList.contains("dark");document.querySelectorAll(".dark-toggle").forEach(t=>{t.textContent=e?"☀":"☾",t.title=e?"Açık mod":"Koyu mod"})}function ce(){const e=document.documentElement.classList.contains("dark");return`<button class="btn btn-ghost dark-toggle" title="${e?"Açık mod":"Koyu mod"}">${e?"☀":"☾"}</button>`}function O(){const e=Q(),t=document.getElementById("app");if(e.page==="upload"||!e.result){t.innerHTML=`
      <header class="app-header">
        <h1>GTFS Analyzer</h1>
        <span style="flex:1"></span>
        ${ce()}
      </header>
      <main id="page-root"></main>`,t.querySelector(".dark-toggle").addEventListener("click",ie),ze(document.getElementById("page-root"));return}const s=Object.keys(ne).map(d=>`
      <button class="nav-btn ${e.page===d?"active":""}" data-page="${d}">
        ${ne[d]}
      </button>`).join("");t.innerHTML=`
    <header class="app-header">
      <h1>GTFS Validator</h1>
      <span class="header-filename">${ht(e.fileName)}</span>
    ...
    <main id="page-root"></main>`,t.querySelector("#btn-back").addEventListener("click",()=>{C("upload"),O()}),t.querySelector("#btn-new").addEventListener("click",()=>{C("upload"),O()}),t.querySelector(".dark-toggle").addEventListener("click",ie),t.querySelectorAll(".nav-btn").forEach(d=>{d.addEventListener("click",()=>{C(d.dataset.page),O()})});const o=document.getElementById("page-root");switch(e.page){case"domain":Oe(o,e.result);break;case"fix":Ve(o,e.result),et(o,e.result,e.result.capped_totals);break;case"rules":st(o,e.result);break;case"export":nt(o,e.result,e.fileName);break}}function ht(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}ut();O();
