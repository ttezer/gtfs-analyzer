...
[![GTFS Spec 根拠文書](https://img.shields.io/badge/GTFS%20Spec-%E6%A0%B9%E6%8B%A0%E6%96%87%E6%9B%B8-007ec6?style=flat)](spec-audit/EVIDENCE_BASE.md)
...
GTFS Validator & Analyzer は、仕様検証を運用品質分析へと拡張します。路線ごとの運行頻度の不整合、異常な速度区間、孤立した停留所、サービスパターンの欠落、ネットワークトポロジーの問題を、593 個の異なる検証・分析ルールで精査します。結果は、準拠性と品質を別々に評価するスコアで要約されます。優先順位付けされた修正キューは、どの問題を最初に対処すべきか、および各修正がスコアに与える可能性のある影響を示します。
...
同じフィードを 2 つのバリデーターで比較しました：MobilityData gtfs-validator v8.0.1・GTFS Analyzer v0.7.0。（GTFS Analyzer の数値は 2026-07-31 に実行した分析のスナップショットです。一部のルールは日付に依存するため、別の日に実行すると小さな差異が生じる場合があります。）

#### BART（ベイエリア高速鉄道、サンフランシスコ）

フィード：`mdb-53`（MobilityDatabase、2026-07-29 スナップショット。有効期間：2026-01-12〜2026-08-30）・14 路線、287 停留所、2,689 便。
...
フィード：`mdb-3175`（MobilityDatabase、2026-07-29 スナップショット。有効期間：2026-07-29〜2029-07-28）・151 路線、5,370 停留所、68,817 便。
...
フィード：`mdb-782`（MobilityDatabase、2026-07-25 スナップショット。有効期間：2026-07-23〜2026-12-12）・1,274 路線、41,961 停留所、258,524 便、14,485 シェイプ・約 75 MB。このフィードは MobilityData のホスト型 Web バリデーターでは処理できないほど大きく、MobilityData の数値はデスクトップアプリケーションで生成されたレポートによるものです。GTFS Analyzer はブラウザで直接（約 15 秒）検証します。
...
> 🇩🇪 **大規模フィード、異なる焦点：** どちらのツールも重大 0 件 — フィードは仕様的にクリーンです。MobilityData の合計の半分以上（`non_ascii_or_non_printable_char`、6,813）は、フィードのドイツ語テキストにある正当な ü/ö/ä/ß 文字です。GTFS Analyzer は有効な Unicode 文字を検出せず、印刷不可／制御文字のみを対象とします。一方 GTFS Analyzer の件数は、MobilityData にない運用的・幾何学的な分析（シェイプ、停留所、統計的所要時間）に由来します。中核的なチェックでは両ツールは一致します：`stop_without_stop_time`（STP_020）と `service_has_no_active_day_of_the_week`（CAL_006）はそれぞれ 1,398 件と 1,035 件で完全一致します。
...
│   ├── rules/      # ルール定義とレジストリ（593 ルール、38 グループ）
