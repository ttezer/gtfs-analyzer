...
同じフィードを 2 つのバリデーターで比較しました：MobilityData gtfs-validator v8.0.1・GTFS Analyzer v0.6.0。（GTFS Analyzer の数値は 2026-07-17 に実行した分析のスナップショットです。一部のルールは日付に依存するため、別の日に実行すると小さな差異が生じる場合があります。）
