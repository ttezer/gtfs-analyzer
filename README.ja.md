...
[![ルール数](https://img.shields.io/badge/rules-621-blue?style=flat)](RULES.ja.md)
...
測定可能な GTFS 仕様要件の **97.2%** をカバーし、フィールドインベントリの 300 個のアトムすべてを少なくとも 1 つの Spec ルールにアンカーしています。**621 個の検証ルール**のうち **426 個**が直近の 4,343 フィード完全カタログ実行で少なくとも 1 件の指摘を出しました。すべてのルールは [`RULES.ja.md`](RULES.ja.md) に一覧化されています。
...
GTFS Validator & Analyzer は、仕様検証を運用品質分析へと拡張します。路線ごとの運行頻度の不整合、異常な速度区間、孤立した停留所、サービスパターンの欠落、ネットワークトポロジーの問題を、621 個の異なる検証・分析ルールで精査します。結果は、準拠性と品質を別々に評価するスコアで要約されます。優先順位付けされた修正キューは、どの問題を最初に対処すべきか、および各修正がスコアに与える可能性のある影響を示します。
...
| **JPN_029** | V4 で推奨される stop_times/attributions のかな読み |
...
同じ検証コア（`gtfs_pipeline::validate_bytes`）を 5 つの方法で実行できます。すべて同じ 621 ルールと同じ結果モデルを使用します。
...
│   ├── rules/      # ルール定義とレジストリ（621 ルール、38 グループ）
