...
同じフィードを 3 つのバリデーターで比較しました：MobilityData gtfs-validator v8.0.1・GTFS Guru v0.1.0・GTFS Analyzer v0.2.0。（GTFS Analyzer の数値は 2026-07-01 に実行した分析のスナップショットです。一部のルールは日付に依存するため、別の日に実行すると小さな差異が生じる場合があります。）
...
> ⚠️ **重複するブロック便：** このフィードの主要な検出は、同一ブロック内で時間的に重複する便です（MobilityData と GTFS Guru で 908 件の*エラー*）。GTFS Analyzer は TRP_022 でこれを検出しますが（770）、同じ日にアクティブなサービスのみを競合としてカウントします（カレンダー交差ガード）。重大度分類はツール間で異なります（Analyzer では致命的ではありません）。
>
