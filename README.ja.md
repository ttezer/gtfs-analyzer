...
GTFS Analyzer は、仕様検証を運用品質分析へと拡張します。路線ごとの運行頻度の不整合、異常な速度区間、孤立した停留所、サービスパターンの欠落、ネットワークトポロジーの問題を、526 個の異なる検証・分析ルールで精査します。結果は、準拠性と品質を別々に評価するスコアで要約されます。優先順位付けされた修正キューは、どの問題を最初に対処すべきか、および各修正がスコアに与える可能性のある影響を示します。
...
同じフィードを 3 つのバリデーターで比較しました：MobilityData gtfs-validator v8.0.1・GTFS Guru v0.1.0・GTFS Analyzer v0.1.3。（GTFS Analyzer の数値は 2026-06-22 に実行した分析のスナップショットです。一部のルールは日付に依存するため、別の日に実行すると小さな差異が生じる場合があります。）

#### BART（ベイエリア高速鉄道、サンフランシスコ）

フィード：`mdb-53`（MobilityDatabase、2026-06-13 スナップショット。有効期間：2026-01-12〜2026-08-07）・14 路線、287 停留所、5,307 便。

| | MobilityData | GTFS Guru | GTFS Analyzer |
|---|---:|---:|---:|
| 総通知数 | 2,733 | 2,663 | 2,218 |
| 致命的 / エラー | 2 | 1 | 2 |
...
フィード：`mdb-247`（MobilityDatabase、2026-06-14 スナップショット。有効期間：2026-05-31〜2026-11-28）・110 路線、6,466 停留所、52,298 便。

| | MobilityData | GTFS Guru | GTFS Analyzer |
|---|---:|---:|---:|
| 総通知数 | 951 | 1,017 | 8,483 |
| 致命的 / エラー | 908 | 908 | 7 |
| 高 / 警告 | 33 | 100 | 1,507 |
| 中 | — | — | 2,849 |
| 低 | — | — | 1,680 |
| 情報 | 10 | 9 | 2,440 |
| 発動した個別ルール種別数 | 10 | 9 | **57** |
| 公開スコア | — | — | **89.3 / 100** |
| 品質スコア | — | — | **51.5 / 100** |

> ⚠️ **重複するブロック便：** このフィードの主要な検出は、同一ブロック内で時間的に重複する便です（MobilityData と GTFS Guru で 908 件の*エラー*）。GTFS Analyzer は TRP_022 でこれを検出しますが（907）、同じ日にアクティブなサービスのみを競合としてカウントします（カレンダー交差ガード）。重大度分類はツール間で異なります（Analyzer では致命的ではありません）。
>
...
フィード：`mdb-3175`（MobilityDatabase、2026-06-21 スナップショット。有効期間：2026-06-21〜2029-06-20）・150 路線、5,367 停留所、57,600 便。

| | MobilityData | GTFS Guru | GTFS Analyzer |
|---|---:|---:|---:|
| 総通知数 | 2,395 | 4,138 | 4,684 |
| 致命的 / エラー | 0 | 0 | 0 |
| 高 / 警告 | 297 | 4,129 | 63 |
| 中 | — | — | 2,090 |
| 低 | — | — | 1,770 |
...
すべての +公開 値の合計は `100 − 現在の公開スコア` に等しく、すべての +品質 値の合計は `100 − 現在の品質スコア` に等しくなります。地理的な問題には地図アイコンが表示され、クリックするとインタラクティブな地図上に問題の場所と関連するシェイプ・停留所データが表示されます。**ルールコード**をクリックすると、関連する GTFS 仕様のセクションが新しいタブで開きます — その指摘が最も影響するファイルのリファレンスページ（GTFS-JP ルールでは gtfs.jp）。
...
│   ├── rules/      # ルール定義とレジストリ（526 ルール、37 グループ）
