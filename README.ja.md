...
同じフィードを 2 つのバリデーターで比較しました：MobilityData gtfs-validator v8.0.1・GTFS Analyzer v0.6.0。（GTFS Analyzer の数値は 2026-07-16 に実行した分析のスナップショットです。一部のルールは日付に依存するため、別の日に実行すると小さな差異が生じる場合があります。）

#### BART（ベイエリア高速鉄道、サンフランシスコ）

フィード：`mdb-53`（MobilityDatabase、2026-06-25 スナップショット。有効期間：2026-01-12〜2026-08-07）・14 路線、287 停留所、5,307 便。
...
フィード：`mdb-247`（MobilityDatabase、2026-07-10 スナップショット。有効期間：2026-06-28〜2026-11-28）・112 路線、6,478 停留所、70,428 便。
...
> ⚠️ **重複するブロック便：** このフィードの主要な検出は、同一ブロック内で時間的に重複する便です（MobilityData で 908 件の*エラー*）。GTFS Analyzer は TRP_022 でこれを検出しますが（770）、同じ日にアクティブなサービスのみを競合としてカウントします（カレンダー交差ガード）。重大度分類はツール間で異なります（Analyzer では致命的ではありません）。
>
...
フィード：`mdb-3175`（MobilityDatabase、2026-07-11 スナップショット。有効期間：2026-07-11〜2029-07-10）・151 路線、5,370 停留所、68,969 便。
