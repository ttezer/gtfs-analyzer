...
[![Nombre de règles](https://img.shields.io/badge/r%C3%A8gles-620-blue?style=flat)](RULES.fr.md)
...
Le projet couvre **97,2 % des exigences mesurables de la spécification GTFS** et rattache les 300 atomes de l’inventaire des champs à au moins une règle Spec. Sur ses **620 règles**, **424** ont produit au moins un signalement lors de la dernière exécution complète sur le catalogue de 4 343 jeux de données. Toutes les règles sont listées dans [`RULES.fr.md`](RULES.fr.md).
...
| **Nombre total de règles** | **178** | **620** |
...
Le même cœur de validation (`gtfs_pipeline::validate_bytes`) s’exécute de cinq manières — toutes utilisent les mêmes 620 règles et produisent le même modèle de résultat :
...
│   ├── rules/      # Définition des règles et registre (620 règles, 38 groupes)
