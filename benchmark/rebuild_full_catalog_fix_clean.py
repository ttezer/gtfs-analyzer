...
# Assumes the five target files were first reset to origin/main.
...
s=s[:start]+'''        let transfer_duration = match parse_u32(&row_map, "transfer_duration") {
            Ok(value) => value,
            Err(err) => {
...
s=s.replace('`parse_u32(row, "transfer_duration")` hata döndürürse veya parse edilen değer `0` ise notice eklenir.','`parse_u32(row, "transfer_duration")` hata döndürürse notice eklenir; `0` dahil tüm `u32` değerleri geçerlidir.')
s=s.replace('- **Yanlış pozitif:** Kod koşulu sağlanıyorsa veri modeli açısından sorun kabul edilir; özel işletme yorumu varsa veri tarafında ayrıca modellenmelidir.','- **Yanlış pozitif:** `0` değerini reddetmek yanlış pozitiftir; GTFS bu sınır değerine izin verir.')
s=s.replace('- `transfer_duration pozitif olmalıdır.`\n- **Çözüm önerisi:** `transfer_duration` değerini pozitif tam sayıya ayarlayın veya boş bırakın.','- Parse edilemeyen/negatif `transfer_duration` geçersizdir.\n- **Çözüm önerisi:** Negatif olmayan tam sayı girin veya boş bırakın.')
...
print('clean surgical patch applied')
