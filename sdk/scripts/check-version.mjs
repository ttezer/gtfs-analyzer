...
const pythonSource = await readFile(new URL('../../pyproject.toml', import.meta.url), 'utf8');
...
const pythonVersion = pythonSource.match(/^version\s*=\s*"([^"]+)"/m)?.[1];

if (!sdkVersion || !engineVersion || !cargoVersion || !pythonVersion) {
  throw new Error('SDK, Python veya engine sürümü kaynaklardan okunamadı.');
}
...
if (pythonVersion !== cargoVersion) {
  throw new Error(`Python paket sürümü ${pythonVersion}, engine sürümü ${cargoVersion}.`);
}
