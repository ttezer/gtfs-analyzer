...
| `Pd6bc0278` | Alan değerleri HTML etiketi, yorum veya kaçış dizisi içermemeli | **KANITLI** | `ARC_032` (`0ed19f93`). ⚠️ **2026-08-07'de GENİŞLETİLDİ (issue #79):** etiket listesi 26 addan **HTML5'in TAMAMINA** çıktı — `<script>`, `<section>`, `<iframe>`, `<style>` gibi etiketler eski listede yoktu ve kuraldan kaçıyordu. Liste hâlâ KAPALI ve bu bilinçli: hüküm "HTML etiketi" der, `<Bilinmiyor>`/`<TBD>` gibi uydurma adlar HTML etiketi DEĞİLDİR ve gerçek veride geçer. Fark, listenin artık standardın ALT KÜMESİ değil KENDİSİ olması. Ölçüm: 20 feed'de yeni bulgu 0 (yanlış pozitif yok). |
...
| `P5f72fb5a` | URL `http://` veya `https://` içeren fully qualified URL olmalı **ve özel karakterler doğru kaçırılmış olmalı** | **KANITLI** | Şema yarısı `cfd8f7d4`. ⚠️ **KAÇIRMA yarısı 2026-08-07'ye kadar KANITSIZDI (issue #80):** `Url::parse` toleranslıdır ve girdiyi NORMALİZE eder — ölçüldü: `a b`→`a%20b`, `a"b`→`a%22b`, `a<b>`→`a%3Cb%3E`, `a\b`→`a/b`, hatta BOZUK `%zz` olduğu gibi kabul ediliyordu. Artık `url_escaping_ok` çıplak özel karakterleri ve bozuk yüzde kaçışını reddediyor. ⚠️ **ASCII dışı REDDEDİLMEZ** (IRI kullanımı yaygın) — bilinçli kalıntı. Ölçüm: 20 feed · 12.897 URL'de ihlal 0. |
