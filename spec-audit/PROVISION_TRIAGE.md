...
| `Pd59e5eaa` | İlk satır alan adlarını içermeli | DOLAYLI | Başlık yerine veri satırı varsa zorunlu sütunlar bulunamaz → `ARC_025` (ya da Fatal). ✅ **KANIT ÇİFTİ ÖLÇÜLDÜ 2026-08-07** (issue #87): `integration.rs::first_line_must_be_a_header_pd59e5eaa` — ihlalli girdi (başlıksız dosya) + **KONTROL girdisi** (başlıklı aynı içerik sessiz). Kanıt `spec-audit/dolayli_evidence.tsv`'de makine-okunur ve `badge_status.py` kapısı eksikse exit 1 verir. Hüküm hâlâ DOLAYLI: ihlali adlandıran kural yok, komşu kural üzerinden ölçülüyor. |
...
| `Pd6bc0278` | Alan değerleri HTML etiketi, yorum veya kaçış dizisi içermemeli | **KANITLI** | `ARC_032` (`0ed19f93`). ⚠️ **İKİ TURDA genişletildi (issue #79).** (1) Etiket listesi 26 addan **HTML5'in TAMAMINA**; `<script>`, `<section>`, `<iframe>`, `<style>` kuraldan kaçıyordu. (2) **5. denetim turu entity tarafını çürüttü:** altı ad tanınıyordu (`nbsp amp lt gt quot apos`) ve `&copy;`, `&reg;`, `&euro;`, `&mdash;` kaçıyordu. Entity tablosu artık **ÜRETİLMİŞ** — WHATWG `entities.json`, 2.125 ad + noktalı virgülsüz de geçerli 106 eski ad; kaynak URL'i ve SHA-256 üretilen dosyada, sürüklenme CI'da.
...
| `P5f72fb5a` | URL `http://` veya `https://` içeren fully qualified URL olmalı **ve özel karakterler doğru kaçırılmış olmalı** | **KANITLI** | Şema yarısı `cfd8f7d4`. ⚠️ **KAÇIRMA yarısı 2026-08-07'ye kadar KANITSIZDI (issue #80):** `Url::parse` toleranslıdır ve girdiyi NORMALİZE eder — ölçüldü: `a b`→`a%20b`, `a"b`→`a%22b`, `a<b>`→`a%3Cb%3E`, `a\b`→`a/b`, hatta BOZUK `%zz` olduğu gibi kabul ediliyordu. Artık `url_escaping_ok` çıplak özel karakterleri ve bozuk yüzde kaçışını reddediyor. 🔴 **5. denetim turu bu kalıntıyı da çürüttü (issue #80):** "ASCII dışı bilinçli kabul" bir ÜRÜN TOLERANSIYDI, spec ölçüsü değil. Hükmün atıf yaptığı W3C URI belgesi (ve RFC 3986) URI'yi ASCII olarak tanımlar; çıplak `ü` **kaçırılmamış** demektir. `url_strict_ok` artık spec'in ölçüsünü uyguluyor; ayrı bir toleranslı predikat EKLENMEDİ (çağıranı olmayacaktı ve iki ölçüyü belirsizleştirirdi). Yüzde kodlanmış eşdeğer (`g%C3%BCzergah`) ve punycode alan adı GEÇERLİDİR. Ölçüm: 20 feed · 12.897 URL — çıplak özel karakter 0 · bozuk yüzde kaçışı 0 · **ASCII dışı 0** · bulgu farkı 0. 🔴 **6. denetim (issue #92) bu KANITLI'yı bir kez daha çürüttü:** predikat doğru ölçüyordu ama **kırpılmış vekil değeri** görüyordu — `" https://example.com "` sert kuralı karşılamıyor sanılıyordu, oysa susuyordu. Alan artık HAM okunuyor (`get_lexical_field`); varlık teşhisi (`"   "` = eksik alan) ayrı okumada. Ölçüm: `mdb-892`'de 137 boşluklu `route_url` → +137 `RTS_005`, hepsi gerçek pozitif. |
...
| `P845d30d4` | stop_sequence | Değerler sefer boyunca artmalı, ardışık olmak zorunda değil | **KANITLI** | Tek kanıt `STM_032` (**Spec**, #97'de Quality'den taşındı). Satırlar denetimden önce sequence'a göre sıralandığı için bu görünümde değerler azalamaz; hükmü ihlal edebilecek tek durum EŞİTLİKTİR, yani hüküm "(trip_id, stop_sequence) benzersiz"e indirgenir — spec bunu ikinci kez `stop_times.txt` başlığında ("Primary key (trip_id, stop_sequence)") söyler. 🔴 **`STM_036` ATFI KALDIRILDI (#97):** o kural DOSYA SIRASINI ölçer (MD `unsorted_stop_times`, INFO) ve değerleri düzgün artan geçerli feed'lerde de ateşler — hükmün kanıtı olamaz. ✅ **İzin tarafı da doğrulandı:** ardışık olmayan `stop_sequence`'e ateşleyen kural YOK — spec'in açık iznini ihlal etmiyoruz (`PTH_017` sınıfı hata aranıp bulunamadı). |
...
| `Pac2e68e4` | stop_access | Geçerli değerler 0/1 | **KANITLI** | `STP_026`; `STP_024` yalnız K2 compatibility parse aralığını ölçer. |
...
| `P7c7134fe` · `P4adfb063` · `P502d68a6` | field_name · record_id · record_sub_id | Diğer tiplerdeki/tablolardaki alanlar çevrilmemeli *(soft)* | **KANITLI** | `TRN_011` (field_name çevrilebilir değil) + `TRN_002`. 🔧 **2026-09-14: iki kuralın yüklemi hükme oturtuldu.** Hüküm TÜR eksenindedir; `TRN_002` el yapımı bir izinli ad listesine, `TRN_011` de alan adında parça aramaya soruyordu. 18. koşumda kuralları ateşleyen 18 feed'in `translations.txt`'lerinin TAMAMI arşivden okundu: `TRN_002`'nin 1.958 bulgusunun 58'i, `TRN_011`'in 50 bulgusunun 50'si yanlış pozitifti (`stops::stop_code` Text, `feed_info::feed_contact_url` URL). `TRN_002` `Kritik·Spec` olduğu için üç feed'i tek başına yayın kapısında tutuyordu. Artık `TRN_002` alanın VARLIĞINI (`known_columns`), `TRN_011` de TÜRÜNÜ (üretilmiş `spec_fields.json` tablosu) sorar. Önem ve sınıf değişmedi: bulguların %97'si gerçek. |
...
**Kapatılmayan sert boşluk: YOK.**

⚠️ `P38cd4e78` (ISO 4217 ondalık) bir süre "bilinçli yazılmayacak" diye kayıtlıydı; gerekçe
2.001.806 eşleşmenin ürünü boğacağıydı. **O gerekçe yanlıştı** ve sorgulanınca çöktü:
gürültü dosya-başına toplulama ile 38 notice'a iniyor (`DQ_016`/`ARC_032` deseni), yayın
riski ise severity ile — `Düşük·Spec`, R1 kapısı saf `Spec ∧ Kritik` olduğu için kuralı hiç
görmüyor. Karar ölçümle değil, **ölçümün yüzeysel okunmasıyla** verilmişti.
